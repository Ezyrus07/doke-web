'use strict';

const r1Recovery = require('./community-realtime-postgres-changes-recovery');

const CONTRACT_ID = 'com-b03b-r2-ephemeral-auth-recovery-readiness-v1';
const REQUIRED_PROJECT_ID = r1Recovery.REQUIRED_PROJECT_ID;
const REQUIRED_BRANCH = r1Recovery.REQUIRED_BRANCH;
const REQUIRED_PULL_REQUEST = r1Recovery.REQUIRED_PULL_REQUEST;
const REQUIRED_AUTHORIZATION_PHRASE = 'I_EXPLICITLY_AUTHORIZE_COM_B03B_R2_EPHEMERAL_AUTH_IDENTITY_LIFECYCLE_AND_REALTIME_SUBSCRIPTION_CANARY_ON_DOKE_STAGING';
const ALLOWED_SCOPE = Object.freeze(['community_posts', 'channel_presence', 'channel_typing']);
const BLOCKED_TOPICS = Object.freeze(['channel_messages']);

const TRANSPORT_PLAN = Object.freeze({
  community_posts: Object.freeze({
    transport: 'authenticated_postgres_changes',
    sourceAuthority: 'public.community_posts',
    publication: 'supabase_realtime',
    publicationMode: 'verify_existing_only',
    rlsRequired: true,
    ephemeralAuthenticatedIdentityRequired: true,
    persistentDomainResidueAllowed: false
  }),
  channel_presence: Object.freeze({
    transport: 'private_presence',
    authorizationTable: 'realtime.messages',
    ephemeralAuthenticatedIdentityRequired: true
  }),
  channel_typing: Object.freeze({
    transport: 'private_broadcast',
    authorizationTable: 'realtime.messages',
    ephemeralAuthenticatedIdentityRequired: true
  }),
  channel_messages: Object.freeze({
    transport: 'blocked',
    blocker: 'REMOTE_CANONICAL_CHANNEL_MESSAGE_AUTHORITY_REQUIRED'
  })
});

function freeze(value) {
  if (!value || typeof value !== 'object' || Object.isFrozen(value)) return value;
  Object.values(value).forEach(freeze);
  return Object.freeze(value);
}

function blocked(reason, extra = {}) {
  return freeze({
    contractId: CONTRACT_ID,
    decision: 'blocked_repository_only',
    reason,
    stagingReadAuthority: false,
    stagingMutationAuthority: false,
    authIdentityLifecycleAuthority: false,
    realtimePublicationMutationAuthority: false,
    realtimeSubscriptionAuthority: false,
    syntheticDomainFixtureAuthority: false,
    persistentResidueAuthority: false,
    productionAuthority: false,
    pullRequestMergeAuthority: false,
    ...extra
  });
}

function evaluateRepositoryRecovery(input = {}) {
  if (input.r1ContractId !== r1Recovery.CONTRACT_ID) return blocked('R1_CONTRACT_MISMATCH');
  if (input.r1AuthorizationConsumed !== true) return blocked('R1_AUTHORIZATION_CONSUMPTION_EVIDENCE_REQUIRED');
  if (input.r1ReusableAfterFailure !== false) return blocked('R1_AUTHORIZATION_MUST_REMAIN_NON_REUSABLE');
  if (input.r1FailureCode !== 'DOKE_COM_B03B_R1_SYNTHETIC_LOGIN_FAILED') return blocked('R1_FAILURE_CLASS_MISMATCH');
  if (input.r1PersistentDomainResidue !== 0) return blocked('R1_ZERO_DOMAIN_RESIDUE_EVIDENCE_REQUIRED');
  if (input.communityPostsPublicationObserved !== true) return blocked('COMMUNITY_POSTS_PUBLICATION_OBSERVATION_REQUIRED');
  if (input.ephemeralAuthLifecyclePrepared !== true) return blocked('EPHEMERAL_AUTH_LIFECYCLE_REQUIRED');
  if (input.sharedCanaryCredentialDependencyRemoved !== true) return blocked('SHARED_CANARY_CREDENTIAL_DEPENDENCY_MUST_BE_REMOVED');
  if (input.zeroResidueCleanupPrepared !== true) return blocked('ZERO_RESIDUE_CLEANUP_REQUIRED');
  if (input.channelMessagesRemoteAuthorityReady === true) return blocked('CHANNEL_MESSAGES_OUT_OF_SCOPE');

  return freeze({
    contractId: CONTRACT_ID,
    decision: 'repository_recovery_ready_new_authorization_required',
    reason: null,
    predecessorContractId: r1Recovery.CONTRACT_ID,
    transportPlan: TRANSPORT_PLAN,
    allowedScope: ALLOWED_SCOPE,
    blockedTopics: BLOCKED_TOPICS,
    sharedCanaryCredentialAllowed: false,
    publicationMutationPlanned: false,
    stagingReadAuthority: false,
    stagingMutationAuthority: false,
    authIdentityLifecycleAuthority: false,
    realtimePublicationMutationAuthority: false,
    realtimeSubscriptionAuthority: false,
    syntheticDomainFixtureAuthority: false,
    persistentResidueAuthority: false,
    productionAuthority: false,
    pullRequestMergeAuthority: false
  });
}

function evaluateStagingRecoveryAuthorization(input = {}) {
  if (input.authorizationPhrase !== REQUIRED_AUTHORIZATION_PHRASE) return blocked('EXPLICIT_COM_B03B_R2_STAGING_AUTHORIZATION_REQUIRED');
  if (input.targetEnvironment !== 'staging') return blocked('STAGING_TARGET_REQUIRED');
  if (input.projectId !== REQUIRED_PROJECT_ID) return blocked('STAGING_PROJECT_MISMATCH');
  if (input.branch !== REQUIRED_BRANCH || input.pullRequest !== REQUIRED_PULL_REQUEST) return blocked('PULL_REQUEST_BOUNDARY_MISMATCH');
  if (input.authorizationConsumed === true || input.executionAttempted === true) return blocked('SINGLE_USE_AUTHORIZATION_ALREADY_CONSUMED');
  if (input.r1AuthorizationReusable !== false) return blocked('R1_AUTHORIZATION_REUSE_PROHIBITED');
  if (input.ephemeralAuthIdentityLifecycleAllowed !== true || input.authIdentityCleanupRequired !== true) return blocked('EPHEMERAL_AUTH_IDENTITY_LIFECYCLE_REQUIRED');
  if (input.sharedCanaryCredentialAllowed === true) return blocked('SHARED_CANARY_CREDENTIAL_PROHIBITED');
  if (input.publicationVerificationRequired !== true) return blocked('COMMUNITY_POSTS_PUBLICATION_VERIFICATION_REQUIRED');
  if (input.publicationMutationAllowed === true) return blocked('R2_PUBLICATION_MUTATION_PROHIBITED');
  if (input.syntheticDomainFixtureLifecycleAllowed !== true || input.syntheticDomainCleanupRequired !== true) return blocked('SYNTHETIC_DOMAIN_FIXTURE_LIFECYCLE_REQUIRED');
  if (input.persistentResidueAllowed === true) return blocked('ZERO_PERSISTENT_RESIDUE_REQUIRED');
  if (input.privateBroadcastPresenceChannelsOnly !== true || input.serverVerifiedSessionRequired !== true) return blocked('AUTHENTICATED_PRIVATE_EPHEMERAL_CHANNELS_REQUIRED');
  if (input.publicRealtimeChannelAllowed === true) return blocked('PUBLIC_REALTIME_CHANNEL_PROHIBITED');
  const scope = Array.isArray(input.scope) ? input.scope.slice().sort() : [];
  const expected = ALLOWED_SCOPE.slice().sort();
  if (JSON.stringify(scope) !== JSON.stringify(expected)) return blocked('COM_B03B_R2_SCOPE_MISMATCH');
  if (scope.includes('channel_messages')) return blocked('CHANNEL_MESSAGES_AUTHORITY_REQUIRED');

  return freeze({
    contractId: CONTRACT_ID,
    decision: 'authorized_for_single_bounded_r2_staging_canary',
    reason: null,
    projectId: REQUIRED_PROJECT_ID,
    branch: REQUIRED_BRANCH,
    pullRequest: REQUIRED_PULL_REQUEST,
    scope: ALLOWED_SCOPE,
    excludedTopics: BLOCKED_TOPICS,
    stagingReadAuthority: true,
    stagingMutationAuthority: true,
    authIdentityLifecycleAuthority: true,
    realtimePublicationMutationAuthority: false,
    realtimePublicationVerificationAuthority: true,
    realtimeSubscriptionAuthority: true,
    syntheticDomainFixtureAuthority: true,
    persistentResidueAuthority: false,
    productionAuthority: false,
    pullRequestMergeAuthority: false,
    singleUse: true,
    reusableAfterFailure: false,
    r1AuthorizationReusable: false
  });
}

module.exports = freeze({
  CONTRACT_ID,
  REQUIRED_PROJECT_ID,
  REQUIRED_BRANCH,
  REQUIRED_PULL_REQUEST,
  REQUIRED_AUTHORIZATION_PHRASE,
  ALLOWED_SCOPE,
  BLOCKED_TOPICS,
  TRANSPORT_PLAN,
  evaluateRepositoryRecovery,
  evaluateStagingRecoveryAuthorization
});
