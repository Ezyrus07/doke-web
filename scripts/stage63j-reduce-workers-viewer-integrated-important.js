...
const backupDir = path.join(root, 'reports/generated/stage63j-workers-viewer-integrated-backup');
const reportPath = path.join(root, 'reports/generated/stage63j-workers-viewer-integrated-important-report.md');
...
const backupPath = path.join(backupDir, 'workers-viewer-integrated.css');
const metaPath = path.join(backupDir, 'stage63j-backup-meta.json');
...
const report = `# Stage 63J — Workers Viewer Integrated Important Reduction\n\n` +
  `## Alvo\n\n- \`${targetRel}\`\n\n` +
  `## Resultado\n\n` +
...
console.log(`[Stage 63J] ${targetRel}`);
