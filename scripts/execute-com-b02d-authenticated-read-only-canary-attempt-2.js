...
const CONFIG = require('../config/com-b02d-authenticated-read-only-canary-attempt-2.json');
const EXPECTED_CONTRACT = 'com-b02d-authenticated-read-only-canary-attempt-2-v1';
...
const REPORT_PATH = path.resolve(
  process.env.COM_B02D_REPORT_PATH ||
  'reports/generated/COM-B02D-AUTHENTICATED-READ-ONLY-CANARY-ATTEMPT-2.json'
);
...
function verifyExecutionEnvelope(env) {
  exact(CONFIG.contractId, EXPECTED_CONTRACT, 'DOKE_COM_B02D_EXECUTION_CONTRACT_MISMATCH');
  exact(CONFIG.attempt, 2, 'DOKE_COM_B02D_ATTEMPT_NUMBER_MISMATCH');
  exact(CONFIG.status, 'explicit_authorization_received_execution_pending', 'DOKE_COM_B02D_EXECUTION_NOT_PENDING');
  ...
  exact(CONFIG.execution.workflowRerunAllowed, false, 'DOKE_COM_B02D_RERUN_MUST_REMAIN_BLOCKED');
  ...
  exact(CONFIG.effects.fixtureCreationAllowed, false, 'DOKE_COM_B02D_FIXTURE_CREATION_BLOCKED');
  exact(CONFIG.effects.sessionCreationAllowed, false, 'DOKE_COM_B02D_SESSION_CREATION_BLOCKED');
...
async function verifyPullRequest(env) {
  const pull = await fetchJson(
    ...
    { headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'doke-com-b02d-attempt-2' } },
...
async function verifyProject(env) {
  const project = await fetchJson(
    `https://api.supabase.com/v1/projects/${REQUIRED_PROJECT_ID}`,
    {
      headers: {
        ...
        'User-Agent': 'doke-com-b02d-attempt-2'
      }
    },
  ...
  if (directHost && directHost !== `db.${REQUIRED_PROJECT_ID}.supabase.co`) {
    fail('DOKE_COM_B02D_DATABASE_HOST_MISMATCH');
  }
...
async function connectStaging(project, password) {
  ...
  for (const host of [...new Set(candidates)]) {
    ...
    const pool = new Pool({
      ...
      application_name: 'doke-com-b02d-read-only-canary-attempt-2'
    });
    try {
...
async function verifySchema(client) {
  const response = await client.query({
    name: 'com-b02d-attempt-2-schema-preflight',
    text: `select
...
function createPgBackedServiceSupabase(client) {
  return Object.freeze({
    async rpc(name, args) {
      ...
      const response = await client.query({
        name: 'com-b02d-attempt-2-load-canonical-state',
...
async function executeCanary(env) {
  ...
  const report = {
    validationId: 'COM-B02D-AUTHENTICATED-READ-ONLY-CANARY-ATTEMPT-2',
  ...
  try {
    ...
    const probePresence = await client.query({
      name: 'com-b02d-attempt-2-probe-absence',
    ...
    writeReport(report);
    console.log('COM-B02D authenticated read-only canary attempt 2 passed.');
