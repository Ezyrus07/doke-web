...
const authorizationReceipt = require('../config/com-b03c-r4v-r4u-fresh-authorization-consumption.json');
...
const r4tBuilder = require('./build-com-b03c-r4t-r4q-successor-executor');

const REPORT_PATH = path.resolve(process.env.COM_B03C_R4W_REPORT_PATH || r4w.REPORT_PATH);
...
function safeFailure(error) {
  const raw = String(error?.code || error?.message || 'DOKE_COM_B03C_R4W_REMOTE_FAILURE');
  return Object.freeze({
    code: /^DOKE_COM_B03C_(?:R3Y|R4G|R4J|R4L|R4N|R4O|R4Q|R4T|R4W)_[A-Z0-9_]+$/.test(raw)
      ? raw
      : 'DOKE_COM_B03C_R4W_REMOTE_FAILURE',
...
function readTrigger(triggerPath = r4w.FUTURE_TRIGGER_PATH) {
  if (!fs.existsSync(triggerPath)) fail(r4w.REMOTE_EXECUTION_BLOCK_CODE);
  try {
    return JSON.parse(fs.readFileSync(triggerPath, 'utf8'));
  } catch {
    fail('DOKE_COM_B03C_R4W_TRIGGER_JSON_INVALID');
  }
}

function assertAuthorizedExecution({ trigger, parentHead, changedFiles, runAttempt } = {}) {
  const authorization = r4w.authorizeFutureTriggerExecution({
    trigger,
    parentHead,
    changedFiles,
    runAttempt,
  ...
  if (authorization.decision !== r4w.AUTHORIZED_DECISION) {
    fail(typeof authorization.reason === 'string'
      ? `DOKE_COM_B03C_R4W_${authorization.reason}`
      : r4w.REMOTE_EXECUTION_BLOCK_CODE);
  }
  return authorization;
}

function prepareRemoteRuntime({ authorization, readCredential, loadDependency } = {}) {
  if (!authorization || authorization.decision !== r4w.AUTHORIZED_DECISION ||
      authorization.executionAttempted !== true || authorization.runAttempt !== 1) {
    fail(r4w.REMOTE_EXECUTION_BLOCK_CODE);
  }
  if (typeof readCredential !== 'function') fail('DOKE_COM_B03C_R4W_CREDENTIAL_READER_REQUIRED');
  if (typeof loadDependency !== 'function') fail('DOKE_COM_B03C_R4W_DEPENDENCY_LOADER_REQUIRED');
  ...
  const credentials = Object.fromEntries(credentialNames.map((name) => {
    ...
    if (!value) fail(`DOKE_COM_B03C_R4W_CREDENTIAL_MISSING_${name}`);
    return [name, value];
  }));
  if (credentials.SUPABASE_PROJECT_REF !== authorizationReceipt.projectId) {
    fail('DOKE_COM_B03C_R4W_PROJECT_REF_MISMATCH');
  }
  ...
  const dependencies = Object.fromEntries(dependencyNames.map((name) => {
    ...
    if (!value) fail(`DOKE_COM_B03C_R4W_DEPENDENCY_MISSING_${name}`);
    return [name, value];
  }));
...
function counterDelta(after, before, key) {
  ...
  if (!Number.isSafeInteger(value) || value < 0) fail('DOKE_COM_B03C_R4W_COUNTER_DELTA_INVALID');
  return value;
}

function baseReport({ trigger } = {}) {
  return {
    schema: r4w.REPORT_SCHEMA,
    validationId: 'COM-B03C-R4W-SINGLE-USE-R4T-SUCCESSOR-HOSTED-EXECUTION',
    contractId: r4o.CONTRACT_ID,
    executionBoundaryContractId: r4w.CONTRACT_ID,
    target: {
      environment: authorizationReceipt.targetEnvironment,
      projectId: authorizationReceipt.projectId,
      branch: authorizationReceipt.branch,
      pullRequest: authorizationReceipt.pullRequest
    },
    phases: [...r4o.PHASES],
    singleUse: true,
    ...
    r4tContractId: r4w.EXECUTION_BINDING.r4tContractId,
    r4tExecutionSemanticsFingerprint: r4w.R4T_EXECUTION_SEMANTICS_FINGERPRINT,
    authorizationReceiptId: r4w.AUTHORIZATION_RECEIPT_ID,
    ...
    runtimePolicyPromotionAuthorized: false,
...
async function executeAuthorizedStaging(env = process.env) {
  ...
  try {
    ...
    if (typeof Pool !== 'function' || typeof createClient !== 'function') {
      fail('DOKE_COM_B03C_R4W_REMOTE_DEPENDENCY_SHAPE_INVALID');
    }
    ...
    const plan = r3v.buildSingleUseExecutionPlan({ ownershipToken: r4w.ownershipTokenForReceipt() });
    if (plan.statementFingerprint !== trigger.statementFingerprint ||
      ...
      fail('DOKE_COM_B03C_R4W_TRIGGER_SQL_BINDING_MISMATCH');
    }
    successorAdapter = r4tBuilder.buildSuccessorDbExecutionAdapter(connection.client, plan);
    db = successorAdapter.db;
    ...
    const realtime = r4lExecutor.buildTerminalRealtimeBridge({
      createClient,
      url: `https://${authorizationReceipt.projectId}.supabase.co`,
    ...
    terminalOutcome = await r4n.runAttributedPhase(recorder, 'realtime_subscribe', async () => {
      ...
      return realtime.runPresenceOnlyProbe({
        ...
        accessToken,
        topic: `room:com-b03c-r4w-${identity.nonce}-presence-only`
      });
    });
    ...
    observation = await r4n.runAttributedPhase(recorder, 'terminal_observation_build', async () => {
      ...
      if (sanitized.decision !== 'terminal_observation_sanitized') {
        fail('DOKE_COM_B03C_R4W_TERMINAL_OBSERVATION_INVALID');
      }
      return sanitized;
    });
    ...
    if (db) {
      try {
        cleanupResult = await r4n.runAttributedPhase(recorder, 'database_cleanup', () =>
          r4tBuilder.executeCleanupPlan({
            db,
            instrumentationInstalled,
            baselinePolicySnapshot,
            policiesEqual: r3yExecutor.policiesEqual
          }));
      } catch (error) {
  ...
  return {
    ...
    successorExecutorKind: successorAdapter?.kind || null,
    successorCodecClientKind: successorAdapter?.codecClientKind || null,
    r4cCounterCodecBridgeApplied: Boolean(successorAdapter),
    instrumentationInstalled,
    presencePolicySwitched,
    cleanupPlan: cleanupResult?.cleanupPlan || null,
    cleanupMutationAttempted: cleanupResult?.cleanupMutationAttempted === true,
    cleanupMutationSucceeded: cleanupResult?.cleanupMutationSucceeded === true,
    residueCounts: cleanupResult?.residueCounts || null,
    zeroResidueProven: cleanupResult?.zeroResidueProven === true,
    baselineRestored: cleanupResult?.baselineRestored === true,
    cleanupRecords: cleanupResult?.cleanupRecords || [],
    ...
    observation,
    executionFailure,
    hostedR4tSuccessorAttemptExecuted: true,
    rawRemoteErrorExposed: false,
    productionChanged: false,
    runtimePolicyPromotionAuthorized: false,
    exactRootCauseProven: false,
...
async function repositorySelfTest() {
  ...
  try {
    ...
    fail('DOKE_COM_B03C_R4W_PREAUTH_HARD_BLOCK_DID_NOT_FIRE');
  } catch (error) {
    if (error?.code !== r4w.REMOTE_EXECUTION_BLOCK_CODE) throw error;
  }
  if (credentialReads !== 0 || dependencyLoads !== 0) {
    fail('DOKE_COM_B03C_R4W_PREAUTH_SIDE_EFFECT_DETECTED');
  }

  const fakeClient = { async query() { return { rows: [], rowCount: 0 }; } };
  const plan = r3v.buildSingleUseExecutionPlan({ ownershipToken: r4w.ownershipTokenForReceipt() });
  const successor = r4tBuilder.buildSuccessorDbExecutionAdapter(fakeClient, plan);
  if (successor.kind !== 'r4t_r4q_successor_db_execution_adapter' || !successor.db) {
    fail('DOKE_COM_B03C_R4W_R4T_SUCCESSOR_ADAPTER_SELF_TEST_FAILED');
  }

  let cleanupMutations = 0;
  let residueInspections = 0;
  let policySnapshots = 0;
  const zeroCounts = Object.fromEntries(r3s.RESIDUE_COUNT_FIELDS.map((field) => [field, 0]));
  const cleanup = await r4tBuilder.executeCleanupPlan({
    db: {
      async cleanup() { cleanupMutations += 1; },
      async inspectResidue() { residueInspections += 1; return zeroCounts; },
      async snapshotPolicies() { policySnapshots += 1; return { complete: true, rows: [] }; }
    },
    instrumentationInstalled: false,
    baselinePolicySnapshot: { complete: true, rows: [] },
    policiesEqual: (left, right) => JSON.stringify(left) === JSON.stringify(right)
  });
  if (cleanupMutations !== 0 || residueInspections !== 1 || policySnapshots !== 1 ||
      cleanup.zeroResidueProven !== true || cleanup.baselineRestored !== true) {
    fail('DOKE_COM_B03C_R4W_R4S_PREINSTALL_CLEANUP_SELF_TEST_FAILED');
  }
  ...
  return {
    ...
    successorExecutorKind: successor.kind,
    successorCodecClientKind: successor.codecClientKind,
    r4cCounterCodecBridgeApplied: true,
    preInstallCleanupMutationAttempted: cleanupMutations > 0,
    preInstallResidueInspectionExecuted: residueInspections === 1,
    preInstallBaselineResnapshotExecuted: policySnapshots === 1,
    zeroResidueProven: false,
    baselineRestored: false,
    hostedR4tSuccessorAttemptExecuted: false,
    rawRemoteErrorExposed: false,
    productionChanged: false,
    runtimePolicyPromotionAuthorized: false,
    exactRootCauseProven: false,
...
if (require.main === module) {
  (async () => {
    ...
    if (!fs.existsSync(r4w.FUTURE_TRIGGER_PATH)) fail(r4w.REMOTE_EXECUTION_BLOCK_CODE);
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      executionFailure: report.executionFailure,
      zeroResidueProven: report.zeroResidueProven,
      baselineRestored: report.baselineRestored,
      rawRemoteErrorExposed: report.rawRemoteErrorExposed,
      exactRootCauseProven: report.exactRootCauseProven
    })}\n`);
    if (report.executionFailure) process.exitCode = 2;
  })().catch((error) => {
    process.stderr.write(`${String(error?.code || error?.message || 'DOKE_COM_B03C_R4W_EXECUTION_FAILED')}\n`);
    process.exitCode = 2;
  });
}
