...
ok('canary contract is materialized but staging stays closed',()=>{assert.equal(c.runtimeCanaryContractAuthorization.scope,'repository_only');assert.equal(c.runtimeCanaryContractAuthorization.authorizationReceived,true);assert.equal(c.runtimeCanaryContractAuthorization.authorizationConsumedForRepositoryArtifacts,true);assert.equal(c.runtimeCanaryContractAuthorization.contractStatus,'repository_ready_identity_and_service_eligibility_remediated_staging_execution_unauthorized');assert.equal(c.runtimeCanaryContractAuthorization.identityAuthority,'seed002-email-resolved');assert.equal(c.runtimeCanaryContractAuthorization.serviceEligibilityAuthority,'approved-service-version');assert.equal(c.runtimeCanaryContractAuthorization.fixtureMode,'synthetic-approved-version');assert.equal(c.authority.stagingCanaryExecutionAuthority,false);});
