...
const REPORT_PATH = path.resolve(process.env.COM_B04I_ATTEMPT_2_REPORT_PATH ||
  'reports/generated/COM-B04I-ATTEMPT-2-STAGING-LIVE-COMPOSITION-ROUTE-CANARY.json');
...
function safeError(error) {
  const code = String(error?.code || error?.message || 'DOKE_COM_B04I_ATTEMPT_2_UNEXPECTED_FAILURE');
  return /^(DOKE|COM)_[A-Z0-9_]+$/.test(code)
    ? { code, message: code }
    : { code: 'DOKE_COM_B04I_ATTEMPT_2_UNEXPECTED_FAILURE', message: 'COM-B04I attempt 2 failed closed.' };
}
...
function verifyEnvelope(env) {
  exact(CONFIG.contractId, 'com-b04i-attempt-2-staging-live-composition-route-canary-v1', 'DOKE_COM_B04I_ATTEMPT_2_CONTRACT_MISMATCH');
  exact(CONFIG.runtimeContractId, CONTRACT_ID, 'DOKE_COM_B04I_ATTEMPT_2_RUNTIME_CONTRACT_MISMATCH');
  exact(CONFIG.status, 'authorization_consumed_execution_pending', 'DOKE_COM_B04I_ATTEMPT_2_NOT_PENDING');
  exact(CONFIG.authorization.phrase, REQUIRED_AUTHORIZATION_PHRASE, 'DOKE_COM_B04I_ATTEMPT_2_AUTHORIZATION_MISMATCH');
  exact(CONFIG.authorization.received, true, 'DOKE_COM_B04I_ATTEMPT_2_AUTHORIZATION_NOT_RECEIVED');
  exact(CONFIG.authorization.consumed, true, 'DOKE_COM_B04I_ATTEMPT_2_AUTHORIZATION_NOT_CONSUMED');
  exact(CONFIG.authorization.singleUse, true, 'DOKE_COM_B04I_ATTEMPT_2_SINGLE_USE_REQUIRED');
  exact(CONFIG.authorization.reusableAfterFailure, false, 'DOKE_COM_B04I_ATTEMPT_2_REUSE_PROHIBITED');
  exact(CONFIG.authorization.sourceHead, REQUIRED_AUTHORIZATION_SOURCE_HEAD, 'DOKE_COM_B04I_ATTEMPT_2_SOURCE_HEAD_MISMATCH');
  exact(CONFIG.execution.attempted, true, 'DOKE_COM_B04I_ATTEMPT_2_EXECUTION_NOT_RECORDED');
  exact(CONFIG.execution.executorStarted, false, 'DOKE_COM_B04I_ATTEMPT_2_EXECUTOR_STATE_INVALID');
  exact(CONFIG.execution.workflowRerunAllowed, false, 'DOKE_COM_B04I_ATTEMPT_2_RERUN_PROHIBITED');
  exact(CONFIG.target.environment, 'staging', 'DOKE_COM_B04I_ATTEMPT_2_TARGET_NOT_STAGING');
  exact(CONFIG.target.projectId, REQUIRED_PROJECT_ID, 'DOKE_COM_B04I_ATTEMPT_2_PROJECT_MISMATCH');
  exact(CONFIG.target.branch, EXPECTED_BRANCH, 'DOKE_COM_B04I_ATTEMPT_2_BRANCH_MISMATCH');
  exact(CONFIG.target.pullRequest, EXPECTED_PR, 'DOKE_COM_B04I_ATTEMPT_2_PR_MISMATCH');
  exact(CONFIG.canary.processLocalActivation, true, 'DOKE_COM_B04I_ATTEMPT_2_PROCESS_LOCAL_REQUIRED');
  exact(CONFIG.canary.publicTrafficEnabled, false, 'DOKE_COM_B04I_ATTEMPT_2_PUBLIC_TRAFFIC_PROHIBITED');
  exact(CONFIG.canary.syntheticOnly, true, 'DOKE_COM_B04I_ATTEMPT_2_SYNTHETIC_ONLY_REQUIRED');
  exact(CONFIG.canary.rollbackOnly, true, 'DOKE_COM_B04I_ATTEMPT_2_ROLLBACK_ONLY_REQUIRED');
  exact(CONFIG.canary.outerIsolation, 'serializable', 'DOKE_COM_B04I_ATTEMPT_2_SERIALIZABLE_REQUIRED');
  assert.deepStrictEqual(CONFIG.canary.rpcAllowlist, REQUIRED_RPC_ALLOWLIST, 'DOKE_COM_B04I_ATTEMPT_2_RPC_ALLOWLIST_MISMATCH');
  exact(env.COM_B04I_ATTEMPT_2_AUTHORIZATION, REQUIRED_AUTHORIZATION_PHRASE, 'DOKE_COM_B04I_ATTEMPT_2_ENV_AUTHORIZATION_MISMATCH');
  exact(env.SUPABASE_PROJECT_REF, REQUIRED_PROJECT_ID, 'DOKE_COM_B04I_ATTEMPT_2_ENV_PROJECT_MISMATCH');
  exact(String(env.GITHUB_RUN_ATTEMPT || '1'), '1', 'DOKE_COM_B04I_ATTEMPT_2_WORKFLOW_RERUN_BLOCKED');
  if (!env.SUPABASE_ACCESS_TOKEN) fail('DOKE_COM_B04I_ATTEMPT_2_ACCESS_TOKEN_MISSING');
  if (!env.SUPABASE_DB_PASSWORD) fail('DOKE_COM_B04I_ATTEMPT_2_DB_PASSWORD_MISSING');
}
...
async function verifyPullRequest(env) {
  const pull = await fetchJson(`https://api.github.com/repos/${EXPECTED_REPOSITORY}/pulls/${EXPECTED_PR}`,
    { headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'doke-com-b04i-attempt-2-canary' } },
    'DOKE_COM_B04I_ATTEMPT_2_PR_PREFLIGHT_FAILED');
  exact(pull.state, 'open', 'DOKE_COM_B04I_ATTEMPT_2_PR_NOT_OPEN');
  exact(pull.draft, true, 'DOKE_COM_B04I_ATTEMPT_2_PR_NOT_DRAFT');
  exact(pull.merged, false, 'DOKE_COM_B04I_ATTEMPT_2_PR_ALREADY_MERGED');
  exact(pull.auto_merge, null, 'DOKE_COM_B04I_ATTEMPT_2_AUTO_MERGE_ENABLED');
  exact(pull.head?.ref, EXPECTED_BRANCH, 'DOKE_COM_B04I_ATTEMPT_2_PR_BRANCH_MISMATCH');
  if (env.GITHUB_SHA) exact(pull.head?.sha, env.GITHUB_SHA, 'DOKE_COM_B04I_ATTEMPT_2_PR_SHA_MISMATCH');
...
async function verifyProject(env) {
  const project = await fetchJson(`https://api.supabase.com/v1/projects/${REQUIRED_PROJECT_ID}`, {
    headers: { Authorization: `Bearer ${env.SUPABASE_ACCESS_TOKEN}`, Accept: 'application/json', 'User-Agent': 'doke-com-b04i-attempt-2-canary' }
  }, 'DOKE_COM_B04I_ATTEMPT_2_PROJECT_PREFLIGHT_FAILED');
  exact(project.id, REQUIRED_PROJECT_ID, 'DOKE_COM_B04I_ATTEMPT_2_PROJECT_ID_MISMATCH');
  exact(project.name, EXPECTED_PROJECT_NAME, 'DOKE_COM_B04I_ATTEMPT_2_PROJECT_NAME_MISMATCH');
  exact(project.status, 'ACTIVE_HEALTHY', 'DOKE_COM_B04I_ATTEMPT_2_PROJECT_NOT_HEALTHY');
  const region = String(project.region || '').trim().toLowerCase();
  if (!/^[a-z]{2}-[a-z]+-\d$/.test(region)) fail('DOKE_COM_B04I_ATTEMPT_2_PROJECT_REGION_INVALID');
  const directHost = project.database?.host || null;
  if (directHost && directHost !== `db.${REQUIRED_PROJECT_ID}.supabase.co`) fail('DOKE_COM_B04I_ATTEMPT_2_DATABASE_HOST_MISMATCH');
...
async function connect(project, password) {
  ...
  for (const host of [...new Set(candidates)]) {
    ...
    const pool = new Pool({
      ...
      application_name: 'doke-com-b04i-attempt-2-staging-route-canary'
    });
    try {
  ...
  fail('DOKE_COM_B04I_ATTEMPT_2_DATABASE_CONNECTION_FAILED');
}

async function verifySchema(client) {
  const result = await client.query({
    name: 'com-b04i-attempt-2-schema-preflight',
    text: `select
  ...
  if (Object.values(result.rows[0] || {}).some((value) => value !== true)) fail('DOKE_COM_B04I_ATTEMPT_2_SCHEMA_GATE_FAILED');
}
...
async function loadActor(client) {
  ...
  if (!row || !['aal1', 'aal2'].includes(row.aal)) fail('DOKE_COM_B04I_ATTEMPT_2_VALID_AUTHENTICATED_SESSION_REQUIRED');
  return {
...
function createExecutor(client) {
  return Object.freeze({
    authority: 'server_service_role', environment: 'staging_rollback_route_canary', rpcAllowlist: REQUIRED_RPC_ALLOWLIST,
    async rpc(name, args) {
      if (!REQUIRED_RPC_ALLOWLIST.includes(name)) fail('DOKE_COM_B04I_ATTEMPT_2_RPC_NOT_ALLOWLISTED');
      try {
        ...
        const result = await client.query({
          name: 'com-b04i-attempt-2-commit-case-command',
        ...
        return { data: result.rows[0]?.data || null, error: null };
      } catch (error) { return { error: { code: String(error?.message || error?.code || 'RPC_FAILURE') } }; }
    }
  });
}
...
function context(actorId) {
  return Object.freeze({
    ...
    community: Object.freeze({ id: CONFIG.synthetic.communityId, status: 'active', source: 'canonical_server', complete: true, revision: 1 }),
    authorization: Object.freeze({ actorId, source: 'canonical_server', complete: true, revision: 1, capabilities: Object.freeze({}) }),
    policy: Object.freeze({
      status: 'approved', version: '2026.08.06-com-b04i-attempt-2', fingerprint: moderation.sha256('com-b04i-attempt-2-approved-canary-policy'),
      automaticEnforcementAllowed: false, reportCountCreatesSanction: false, scanResultCreatesFinalDecision: false
    }),
    target: Object.freeze({ id: CONFIG.synthetic.targetId, communityId: CONFIG.synthetic.communityId, ownerId: actorId, type: 'community_post', state: 'published', source: 'canonical_server', complete: true, revision: 1 })
  });
}
...
function assertDelta(before, inside) {
  ...
  for (const [key, delta] of Object.entries(expected)) exact(inside[key] - before[key], delta, `DOKE_COM_B04I_ATTEMPT_2_COUNT_DELTA_${key.toUpperCase()}_INVALID`);
  return expected;
}

async function run(env) {
  const report = {
    validationId: 'COM-B04I-ATTEMPT-2-STAGING-LIVE-COMPOSITION-ROUTE-CANARY', contractId: CONFIG.contractId, runtimeContractId: CONTRACT_ID, domain: 'COM-001',
    status: 'execution_started', authorization: { phraseSha256: hash(REQUIRED_AUTHORIZATION_PHRASE), sourceHead: REQUIRED_AUTHORIZATION_SOURCE_HEAD, consumed: true, singleUse: true, reusableAfterFailure: false },
    target: { environment: 'staging', projectId: REQUIRED_PROJECT_ID }, startedAt: new Date().toISOString(), completedAt: null,
  ...
  try {
    ...
    await client.query('BEGIN ISOLATION LEVEL SERIALIZABLE READ WRITE'); open = true;
    ...
    if (!serverNow || !Number.isFinite(Date.parse(serverNow))) fail('DOKE_COM_B04I_ATTEMPT_2_SERVER_CLOCK_REQUIRED');
    ...
    const transactionGuard = Object.freeze({
      ...
      async assertActive() {
        ...
        exact(result.rows[0]?.current_user, 'service_role', 'DOKE_COM_B04I_ATTEMPT_2_SERVICE_ROLE_NOT_ACTIVE');
        exact(result.rows[0]?.isolation, 'serializable', 'DOKE_COM_B04I_ATTEMPT_2_TRANSACTION_NOT_SERIALIZABLE');
        exact(result.rows[0]?.read_only, 'off', 'DOKE_COM_B04I_ATTEMPT_2_TRANSACTION_READ_ONLY_INVALID');
        return true;
      }
    });
    const runtime = createModerationStagingLiveRouteCanary({
      authorization: CONFIG.authorization, transactionGuard, executor: createExecutor(client),
      sessionVerifier: Object.freeze({ authority: 'server_verified_session_boundary', async verify() { return authenticated.session; } }),
      contextLoader: Object.freeze({ authority: 'canonical_server_context_loader', async load({ actorId }) { return context(actorId); } }),
    ...
    const response = await handler({
      headers: Object.freeze({}), requestId: 'com-b04i-attempt-2-staging-live-route-canary',
      envelope: Object.freeze({
        command: 'open_case', clientRequestId: CONFIG.synthetic.clientRequestId, expectedRevision: 0,
        payload: Object.freeze({ kind: 'content_report', initialEvidenceKind: 'report_statement',
    ...
    exact(response.status, 200, 'DOKE_COM_B04I_ATTEMPT_2_ROUTE_STATUS_INVALID');
    exact(response.body.activationMode, 'staging_authenticated_server_runtime', 'DOKE_COM_B04I_ATTEMPT_2_ACTIVATION_MODE_INVALID');
    exact(response.body.authenticatedSessionVerified, true, 'DOKE_COM_B04I_ATTEMPT_2_SESSION_NOT_VERIFIED');
    exact(response.body.rawIdentifiersExposed, false, 'DOKE_COM_B04I_ATTEMPT_2_RAW_IDENTIFIER_EXPOSURE');
    ...
    const evidence = await client.query({
      name: 'com-b04i-attempt-2-inside-transaction-evidence',
      text: `select projection.revision, projection.ledger_head_hash, evidence.evidence_kind, evidence.opaque_reference, evidence.digest
        ...
        where encode(digest(projection.case_id::text, 'sha256'), 'hex') = $1`, values: [caseHash]
    });
    const row = evidence.rows[0];
    if (!row) fail('DOKE_COM_B04I_ATTEMPT_2_ROLLBACK_SCOPED_ROWS_MISSING');
    exact(Number(row.revision), 1, 'DOKE_COM_B04I_ATTEMPT_2_REVISION_INVALID');
    exact(row.ledger_head_hash, response.body.eventHash, 'DOKE_COM_B04I_ATTEMPT_2_LEDGER_HASH_INVALID');
    exact(row.evidence_kind, 'report_statement', 'DOKE_COM_B04I_ATTEMPT_2_EVIDENCE_KIND_INVALID');
    exact(row.opaque_reference, CONFIG.synthetic.initialEvidenceReference, 'DOKE_COM_B04I_ATTEMPT_2_EVIDENCE_REFERENCE_INVALID');
    exact(row.digest, moderation.sha256(CONFIG.synthetic.initialEvidenceDigestSeed), 'DOKE_COM_B04I_ATTEMPT_2_EVIDENCE_DIGEST_INVALID');

    await client.query('ROLLBACK'); open = false;
    const after = await counts(client);
    assert.deepStrictEqual(after, baseline, 'DOKE_COM_B04I_ATTEMPT_2_PERSISTENT_COUNTS_CHANGED');
    const residue = await client.query({
      name: 'com-b04i-attempt-2-postflight-residue',
      text: `select
        ...
        not exists (select 1 from com_moderation_private.evidence_record where encode(digest(case_id::text, 'sha256'), 'hex') = $1) as evidence_absent`, values: [caseHash]
    });
    if (Object.values(residue.rows[0] || {}).some((value) => value !== true)) fail('DOKE_COM_B04I_ATTEMPT_2_PERSISTENT_RESIDUE_DETECTED');

    report.status = 'authenticated_process_local_live_route_canary_passed';
    report.result = {
      authenticatedSessionVerified:true, actorSource:authenticated.evidence.source, assuranceLevel:authenticated.evidence.aal,
      routeName:response.body.routeName, routeStatus:response.status, activationMode:response.body.activationMode,
      domainDecision:response.body.decision, transactionIsolation:'serializable', transactionRolledBack:true,
      revisionObservedInsideTransaction:response.body.revision, eventHash:response.body.eventHash, caseIdSha256:caseHash,
      countDeltaInsideTransaction:delta, countsRestoredAfterRollback:true, persistentResidue:false, rawIdentifiersExposed:false,
      publicTrafficEnabled:false, runtimeDeployed:false
    };
    report.effects.persistentMutationExecuted = false;
    report.completedAt = new Date().toISOString(); writeReport(report);
    console.log('COM-B04I attempt-2 authenticated process-local live composition route canary passed.');
    ...
    report.status = 'failed_closed'; report.failure = safeError(error);
    ...
    report.effects.persistentMutationExecuted = false; report.completedAt = new Date().toISOString(); writeReport(report); throw error;
  } finally {
...
run(process.env).catch((error) => { console.error(safeError(error).code); process.exit(1); });
