...
check('low cardinality rollups',sql.includes("('analytics_projection_missing_rate'")&&sql.includes("('analytics_reconciliation_mismatch_rate'"));check('dimension mismatch no longer synthetic zero',sql.includes("'dimensionMismatch',v_dimension_mismatch")&&!sql.includes("'dimensionMismatch',0"));check('mismatch rate bounded by event universe',sql.includes('v_mismatch_subject_count::numeric / v_reconciliation_universe_count')&&sql.includes("'reconciliationUniverseCount',v_reconciliation_universe_count"));
