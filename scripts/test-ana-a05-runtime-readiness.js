...
check('low cardinality rollups',sql.includes("('analytics_projection_missing_rate'")&&sql.includes("('analytics_reconciliation_mismatch_rate'"));check('event key mismatch is reconciled',sql.includes("'eventKeyMismatch',v_event_key_mismatch")&&sql.includes('s.event_key is distinct from p.event_key'));check('dimension mismatch no longer synthetic zero',sql.includes("'dimensionMismatch',v_dimension_mismatch")&&!sql.includes("'dimensionMismatch',0")&&sql.includes("o.service_snapshot ->> 'category'"));check('mismatch rate bounded by event universe',sql.includes('v_mismatch_subject_count::numeric / v_reconciliation_universe_count')&&sql.includes("'reconciliationUniverseCount',v_reconciliation_universe_count"));
