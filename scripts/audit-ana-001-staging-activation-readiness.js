...
const runtimePolicy = config.canonicalRuntimePolicy || {};
const runtimeValues = runtimePolicy.values || {};
check('runtime policy version', runtimePolicy.version === 'ana-runtime-policy-v1');
check('runtime policy is technical not retention', runtimePolicy.classification === 'technical_security_and_integrity_not_data_retention');
check('session ttl canonical', runtimeValues.DOKE_ANALYTICS_SESSION_TTL_SECONDS === 1800);
check('quote ttl canonical', runtimeValues.DOKE_ANALYTICS_QUOTE_SESSION_TTL_SECONDS === 3600);
check('exposure ttl canonical', runtimeValues.DOKE_ANALYTICS_EXPOSURE_TTL_SECONDS === 300);
check('rate policy canonical', runtimeValues.DOKE_ANALYTICS_RATE_LIMIT === 120 && runtimeValues.DOKE_ANALYTICS_RATE_WINDOW_SECONDS === 60);
check('dedup window canonical', runtimeValues.DOKE_ANALYTICS_DEDUP_WINDOW_SECONDS === 60);
check('persistent anonymous identity forbidden', runtimePolicy.privacyBoundary && runtimePolicy.privacyBoundary.persistentAnonymousIdentity === false);
check('anonymous auth stitching forbidden', runtimePolicy.privacyBoundary && runtimePolicy.privacyBoundary.anonymousToAuthenticatedStitching === false);
check('retention remains legal blocker', runtimePolicy.privacyBoundary && runtimePolicy.privacyBoundary.dataRetentionPolicyDefined === false && runtimePolicy.privacyBoundary.legalAuthority === 'LEGAL-B03');
check('secrets separated', runtimePolicy.secretPolicy && runtimePolicy.secretPolicy.distinctSessionAndExposureSecrets === true);
check('secrets not stored in repository', runtimePolicy.secretPolicy && runtimePolicy.secretPolicy.repositoryStorageForbidden === true);
