...
check('projection grant keeps snapshot publication gated',c.validationPlan?.metricSpecificThresholds==='active_r1_360_seconds'&&c.snapshotHandoff?.blockingReason==='RUNTIME_SNAPSHOT_PUBLICATION_AUTHORITY_PENDING'&&c.snapshotHandoff?.publicationAllowed===false&&c.authority?.runtimeProjectionAuthority===true&&c.runtime?.runtimeAuthority===false&&c.runtime?.snapshotPublicationAllowed===false&&c.runtimeProjectionAuthorityGrant?.stagingMutationAuthorized===false);
...
check('segmentation candidate remains unapplied',c.segmentationRuntimeCandidate?.migrationCreated===true&&c.segmentationRuntimeCandidate?.migrationApplied===false&&c.segmentationRuntimeCandidate?.stagingValidated===false&&c.segmentationRuntimeCandidate?.validation047Status==='pending'&&c.segmentationRuntimeCandidate?.runtimeSegmentationAuthority===false&&c.segmentationRuntimeCandidate?.runtimeSnapshotAuthority===false&&c.segmentationRuntimeCandidate?.snapshotPublicationAuthority===false&&c.segmentationRuntimeCandidate?.globalRuntimeUnchanged===true);
