...
const CONFIG = require('../config/com-b04i-staging-live-composition-route-canary.json');
...
const REPORT_PATH = path.resolve(process.env.COM_B04I_REPORT_PATH ||
  'reports/generated/COM-B04I-STAGING-LIVE-COMPOSITION-ROUTE-CANARY.json');
...
function safeError(error) {
  const code = String(error?.code || error?.message || 'DOKE_COM_B04I_UNEXPECTED_FAILURE');
  return /^(DOKE|COM)_[A-Z0-9_]+$/.test(code)
    ? { code, message: code }
    : { code: 'DOKE_COM_B04I_UNEXPECTED_FAILURE', message: 'COM-B04I failed closed.' };
}
...
function verifyEnvelope(env) {
  exact(CONFIG.contractId, CONTRACT_ID, 'DOKE_COM_B04I_CONTRACT_MISMATCH');
  exact(CONFIG.status, 'authorization_consumed_execution_pending', 'DOKE_COM_B04I_NOT_PENDING');
  exact(CONFIG.authorization.phrase, REQUIRED_AUTHORIZATION_PHRASE, 'DOKE_COM_B04I_AUTHORIZATION_MISMATCH');
  exact(CONFIG.authorization.received, true, 'DOKE_COM_B04I_AUTHORIZATION_NOT_RECEIVED');
  exact(CONFIG.authorization.consumed, true, 'DOKE_COM_B04I_AUTHORIZATION_NOT_CONSUMED');
  exact(CONFIG.authorization.singleUse, true, 'DOKE_COM_B04I_SINGLE_USE_REQUIRED');
  exact(CONFIG.authorization.reusableAfterFailure, false, 'DOKE_COM_B04I_REUSE_PROHIBITED');
  exact(CONFIG.authorization.sourceHead, '20b17d96012756c547d522dd5ae38637185de4ee', 'DOKE_COM_B04I_SOURCE_HEAD_MISMATCH');
  exact(CONFIG.execution.attempted, true, 'DOKE_COM_B04I_EXECUTION_NOT_RECORDED');
  exact(CONFIG.execution.workflowRerunAllowed, false, 'DOKE_COM_B04I_RERUN_PROHIBITED');
  exact(CONFIG.target.environment, 'staging', 'DOKE_COM_B04I_TARGET_NOT_STAGING');
  exact(CONFIG.target.projectId, REQUIRED_PROJECT_ID, 'DOKE_COM_B04I_PROJECT_MISMATCH');
  exact(CONFIG.target.branch, EXPECTED_BRANCH, 'DOKE_COM_B04I_BRANCH_MISMATCH');
  exact(CONFIG.target.pullRequest, EXPECTED_PR, 'DOKE_COM_B04I_PR_MISMATCH');
  exact(CONFIG.canary.processLocalActivation, true, 'DOKE_COM_B04I_PROCESS_LOCAL_REQUIRED');
  exact(CONFIG.canary.publicTrafficEnabled, false, 'DOKE_COM_B04I_PUBLIC_TRAFFIC_PROHIBITED');
  exact(CONFIG.canary.syntheticOnly, true, 'DOKE_COM_B04I_SYNTHETIC_ONLY_REQUIRED');
  exact(CONFIG.canary.rollbackOnly, true, 'DOKE_COM_B04I_ROLLBACK_ONLY_REQUIRED');
  exact(CONFIG.canary.outerIsolation, 'serializable', 'DOKE_COM_B04I_SERIALIZABLE_REQUIRED');
  assert.deepStrictEqual(CONFIG.canary.rpcAllowlist, REQUIRED_RPC_ALLOWLIST, 'DOKE_COM_B04I_RPC_ALLOWLIST_MISMATCH');
  exact(env.COM_B04I_AUTHORIZATION, REQUIRED_AUTHORIZATION_PHRASE, 'DOKE_COM_B04I_ENV_AUTHORIZATION_MISMATCH');
  exact(env.SUPABASE_PROJECT_REF, REQUIRED_PROJECT_ID, 'DOKE_COM_B04I_ENV_PROJECT_MISMATCH');
  exact(String(env.GITHUB_RUN_ATTEMPT || '1'), '1', 'DOKE_COM_B04I_WORKFLOW_RERUN_BLOCKED');
  if (!env.SUPABASE_ACCESS_TOKEN) fail('DOKE_COM_B04I_ACCESS_TOKEN_MISSING');
  if (!env.SUPABASE_DB_PASSWORD) fail('DOKE_COM_B04I_DB_PASSWORD_MISSING');
}
...
async function verifyPullRequest(env) {
  const pull = await fetchJson(`https://api.github.com/repos/${EXPECTED_REPOSITORY}/pulls/${EXPECTED_PR}`,
    { headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'doke-com-b04i-canary' } },
    'DOKE_COM_B04I_PR_PREFLIGHT_FAILED');
  exact(pull.state, 'open', 'DOKE_COM_B04I_PR_NOT_OPEN');
  exact(pull.draft, true, 'DOKE_COM_B04I_PR_NOT_DRAFT');
  exact(pull.merged, false, 'DOKE_COM_B04I_PR_ALREADY_MERGED');
  exact(pull.auto_merge, null, 'DOKE_COM_B04I_AUTO_MERGE_ENABLED');
  exact(pull.head?.ref, EXPECTED_BRANCH, 'DOKE_COM_B04I_PR_BRANCH_MISMATCH');
  if (env.GITHUB_SHA) exact(pull.head?.sha, env.GITHUB_SHA, 'DOKE_COM_B04I_PR_SHA_MISMATCH');
...
async function verifyProject(env) {
  const project = await fetchJson(`https://api.supabase.com/v1/projects/${REQUIRED_PROJECT_ID}`, {
    headers: { Authorization: `Bearer ${env.SUPABASE_ACCESS_TOKEN}`, Accept: 'application/json', 'User-Agent': 'doke-com-b04i-canary' }
  }, 'DOKE_COM_B04I_PROJECT_PREFLIGHT_FAILED');
  exact(project.id, REQUIRED_PROJECT_ID, 'DOKE_COM_B04I_PROJECT_ID_MISMATCH');
  exact(project.name, EXPECTED_PROJECT_NAME, 'DOKE_COM_B04I_PROJECT_NAME_MISMATCH');
  exact(project.status, 'ACTIVE_HEALTHY', 'DOKE_COM_B04I_PROJECT_NOT_HEALTHY');
  const region = String(project.region || '').trim().toLowerCase();
  if (!/^[a-z]{2}-[a-z]+-\d$/.test(region)) fail('DOKE_COM_B04I_PROJECT_REGION_INVALID');
  const directHost = project.database?.host || null;
  if (directHost && directHost !== `db.${REQUIRED_PROJECT_ID}.supabase.co`) fail('DOKE_COM_B04I_DATABASE_HOST_MISMATCH');
...
async function connect(project, password) {
  ...
  for (const host of [...new Set(candidates)]) {
    ...
    const pool = new Pool({
      ...
      application_name: 'doke-com-b04i-staging-route-canary'
    });
    try {
  ...
  fail('DOKE_COM_B04I_DATABASE_CONNECTION_FAILED');
}

async function verifySchema(client) {
  const result = await client.query({
    name: 'com-b04i-schema-preflight',
    text: `select
  ...
  if (Object.values(result.rows[0] || {}).some((value) => value !== true)) fail('DOKE_COM_B04I_SCHEMA_GATE_FAILED');
}
...
async function loadActor(client) {
  ...
  if (!row || !['aal1', 'aal2'].includes(row.aal)) fail('DOKE_COM_B04I_VALID_AUTHENTICATED_SESSION_REQUIRED');
  return {
...
function createExecutor(client) {
  return Object.freeze({
    ...
    async rpc(name, args) {
      if (!REQUIRED_RPC_ALLOWLIST.includes(name)) fail('DOKE_COM_B04I_RPC_NOT_ALLOWLISTED');
      try {
        ...
        const result = await client.query({
          name: 'com-b04i-commit-case-command',
...
function context(actorId) {
  return Object.freeze({
    ...
    policy: Object.freeze({
      status: 'approved', version: '2026.08.06-com-b04i-canary', fingerprint: moderation.sha256('com-b04i-approved-canary-policy'),
...
function assertDelta(before, inside) {
  ...
  for (const [key, delta] of Object.entries(expected)) exact(inside[key] - before[key], delta, `DOKE_COM_B04I_COUNT_DELTA_${key.toUpperCase()}_INVALID`);
  return expected;
}

async function run(env) {
  const report = {
    validationId: 'COM-B04I-STAGING-LIVE-COMPOSITION-ROUTE-CANARY', contractId: CONTRACT_ID, domain: 'COM-001',
    status: 'execution_started', authorization: { phraseSha256: hash(REQUIRED_AUTHORIZATION_PHRASE), consumed: true, singleUse: true, reusableAfterFailure: false },
  ...
  try {
    ...
    if (!serverNow || !Number.isFinite(Date.parse(serverNow))) fail('DOKE_COM_B04I_SERVER_CLOCK_REQUIRED');
    ...
    const transactionGuard = Object.freeze({
      ...
      async assertActive() {
        ...
        exact(result.rows[0]?.current_user, 'service_role', 'DOKE_COM_B04I_SERVICE_ROLE_NOT_ACTIVE');
        exact(result.rows[0]?.isolation, 'serializable', 'DOKE_COM_B04I_TRANSACTION_NOT_SERIALIZABLE');
        exact(result.rows[0]?.read_only, 'off', 'DOKE_COM_B04I_TRANSACTION_READ_ONLY_INVALID');
        return true;
      }
    });
    ...
    const response = await handler({
      headers: Object.freeze({}), requestId: 'com-b04i-staging-live-route-canary',
    ...
    exact(response.status, 200, 'DOKE_COM_B04I_ROUTE_STATUS_INVALID');
    exact(response.body.activationMode, 'staging_authenticated_server_runtime', 'DOKE_COM_B04I_ACTIVATION_MODE_INVALID');
    exact(response.body.authenticatedSessionVerified, true, 'DOKE_COM_B04I_SESSION_NOT_VERIFIED');
    exact(response.body.rawIdentifiersExposed, false, 'DOKE_COM_B04I_RAW_IDENTIFIER_EXPOSURE');
    ...
    const evidence = await client.query({
      name: 'com-b04i-inside-transaction-evidence',
    ...
    if (!row) fail('DOKE_COM_B04I_ROLLBACK_SCOPED_ROWS_MISSING');
    exact(Number(row.revision), 1, 'DOKE_COM_B04I_REVISION_INVALID');
    exact(row.ledger_head_hash, response.body.eventHash, 'DOKE_COM_B04I_LEDGER_HASH_INVALID');
    exact(row.evidence_kind, 'report_statement', 'DOKE_COM_B04I_EVIDENCE_KIND_INVALID');
    exact(row.opaque_reference, CONFIG.synthetic.initialEvidenceReference, 'DOKE_COM_B04I_EVIDENCE_REFERENCE_INVALID');
    exact(row.digest, moderation.sha256(CONFIG.synthetic.initialEvidenceDigestSeed), 'DOKE_COM_B04I_EVIDENCE_DIGEST_INVALID');

    await client.query('ROLLBACK'); open = false;
    const after = await counts(client);
    assert.deepStrictEqual(after, baseline, 'DOKE_COM_B04I_PERSISTENT_COUNTS_CHANGED');
    const residue = await client.query({
      name: 'com-b04i-postflight-residue',
      text: `select
    ...
    if (Object.values(residue.rows[0] || {}).some((value) => value !== true)) fail('DOKE_COM_B04I_PERSISTENT_RESIDUE_DETECTED');
    ...
    console.log('COM-B04I authenticated process-local live composition route canary passed.');
