...
const edge=fs.readFileSync(path.join(root,'supabase/functions/search-public-services-v2/index.ts'),'utf8');check('search getUser pattern',edge.includes('getUser'));check('search rate limit pattern',edge.includes('enforceActorRateLimit'));check('search service role pattern',edge.includes('serviceClient'));
