...
for (const file of htmlFiles) {
  ...
  const scripts = extractScripts(html).filter((script) => script.normalizedSrc === DRAWER_SRC || script.normalizedSrc === LEGACY_DRAWER_SRC);
  for (const script of scripts) {
    usages.push({
      ...
      currentOwnerPath: script.normalizedSrc === DRAWER_SRC ? 'ui' : 'pages/home',
...
const report = {
  ...
  summary: {
    ...
    risk: usages.some((usage) => usage.normalizedSrc === LEGACY_DRAWER_SRC) ? 'legacy-path-still-used' : 'shared-ui-path-active',
