...
const FIXTURE_PATH = 'supabase/seed/007_backend_real_admin_projection.sql';
const EXECUTE_CONFIRMATION = 'apply-backend-real-admin-projection';
const EXPECTED_USER = Object.freeze({
  email: 'admin@doke.local',
  role: 'admin',
  status: 'active'
});
...
main().catch((error) => {
  console.error(`Backend Real admin projection failed: ${sanitize(error && error.message)}`);
  process.exitCode = 1;
});
...
async function main() {
  ...
  if (checkEnv) {
    console.log('Backend Real admin projection environment is valid. No network request or mutation was made.');
  ...
  const client = new Client({
    ...
    application_name: 'doke-backend-real-admin-projection'
  });
  ...
  try {
    ...
    const admin = await assertProjection(client);

    if (dryRun) {
      await client.query('rollback');
      transactionOpen = false;
      console.log('Backend Real admin projection dry-run passed. Transaction was rolled back; staging data was not changed.');
      printSafeSummary(config, 'dry-run', admin.id);
      return;
    }

    await client.query('commit');
    transactionOpen = false;
    console.log('Backend Real admin projection applied successfully.');
    printSafeSummary(config, 'execute', admin.id);
  } catch (error) {
...
function readConfig() {
  return Object.freeze({
    ...
    confirmation: readEnv('DOKE_BACKEND_REAL_ADMIN_PROJECTION_CONFIRM'),
    allowMutation: readEnv('DOKE_BACKEND_REAL_ADMIN_PROJECTION_ALLOW_MUTATION')
  });
}
...
function validateExecutionConsent(config) {
  if (config.allowMutation !== '1') {
    throw new Error('DOKE_BACKEND_REAL_ADMIN_PROJECTION_ALLOW_MUTATION=1 is required for --execute.');
  }
  if (config.confirmation !== EXECUTE_CONFIRMATION) {
    throw new Error(`DOKE_BACKEND_REAL_ADMIN_PROJECTION_CONFIRM must equal ${EXECUTE_CONFIRMATION}.`);
  }
}
...
async function assertProjection(client) {
  const result = await client.query(
    `select public_user.id, lower(public_user.email) as email, public_user.role, public_user.status
       from public.users public_user
       join auth.users auth_user on auth_user.id = public_user.id
      where lower(auth_user.email) = $1
        and lower(public_user.email) = $1`,
    [EXPECTED_USER.email]
  );
  if (result.rowCount !== 1) {
    throw new Error('Projection verification did not find the expected admin public user.');
  }
  ...
  if (
    ...
    throw new Error('Projection verification found unexpected fields for admin.');
  }
  return actual;
}

function printSafeSummary(config, mode, adminId) {
  console.log(`- mode: ${mode}`);
  console.log(`- environment: ${config.environment}`);
  console.log(`- project ref: ${config.projectRef}`);
  console.log(`- fixture: ${FIXTURE_PATH}`);
  console.log(`- ${EXPECTED_USER.role}: id=${adminId || 'resolved-from-auth-users'}; email=${EXPECTED_USER.email}; status=${EXPECTED_USER.status}`);
}
