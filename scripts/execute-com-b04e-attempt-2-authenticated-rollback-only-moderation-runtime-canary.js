...
const EXPECTED_CONTRACT = 'com-b04e-attempt-2-authenticated-rollback-only-moderation-runtime-composition-canary-v1';
...
const REPORT_PATH = path.resolve(
  process.env.COM_B04E_ATTEMPT_2_REPORT_PATH ||
  'reports/generated/COM-B04E-ATTEMPT-2-AUTHENTICATED-ROLLBACK-ONLY-MODERATION-RUNTIME-CANARY.json'
);
...
function safeError(error) {
  const code = String(error && error.code || 'DOKE_COM_B04E_ATTEMPT_2_UNEXPECTED_FAILURE');
  if (/^(DOKE|COM)_[A-Z0-9_]+$/.test(code)) return { code, message: code };
  return {
    code: 'DOKE_COM_B04E_ATTEMPT_2_UNEXPECTED_FAILURE',
    message: 'The COM-B04E attempt-2 authenticated rollback-only canary failed closed.',
...
function verifyAttempt1Closure() {
  exact(ATTEMPT_1.status, 'authorization_consumed_pre_execution_audit_failed', 'DOKE_COM_B04E_ATTEMPT_1_STATUS_INVALID');
  exact(ATTEMPT_1.authorization.consumed, true, 'DOKE_COM_B04E_ATTEMPT_1_NOT_CONSUMED');
  exact(ATTEMPT_1.authorization.consumedByRun, 31065331290, 'DOKE_COM_B04E_ATTEMPT_1_RUN_MISMATCH');
  exact(ATTEMPT_1.authorization.reusableAfterFailure, false, 'DOKE_COM_B04E_ATTEMPT_1_REUSE_ENABLED');
  exact(ATTEMPT_1.execution.attempted, true, 'DOKE_COM_B04E_ATTEMPT_1_NOT_RECORDED');
  exact(ATTEMPT_1.execution.executorStarted, false, 'DOKE_COM_B04E_ATTEMPT_1_EXECUTOR_STATE_INVALID');
  exact(ATTEMPT_1.execution.databaseConnectionAttemptedByWorkflow, false, 'DOKE_COM_B04E_ATTEMPT_1_DATABASE_STATE_INVALID');
  exact(ATTEMPT_1.execution.rollbackScopedMutationExecuted, false, 'DOKE_COM_B04E_ATTEMPT_1_MUTATION_STATE_INVALID');
  exact(ATTEMPT_1.postflight.persistentResidue, false, 'DOKE_COM_B04E_ATTEMPT_1_RESIDUE_INVALID');
  for (const key of [
    'caseProjectionRows',
    'caseEventRows',
  ...
  ]) exact(ATTEMPT_1.postflight[key], 0, `DOKE_COM_B04E_ATTEMPT_1_${key.toUpperCase()}_INVALID`);
}

function verifyExecutionEnvelope(env) {
  verifyAttempt1Closure();
  exact(CONFIG.contractId, EXPECTED_CONTRACT, 'DOKE_COM_B04E_ATTEMPT_2_EXECUTION_CONTRACT_MISMATCH');
  exact(CONFIG.status, 'explicit_authorization_received_execution_pending', 'DOKE_COM_B04E_ATTEMPT_2_EXECUTION_NOT_PENDING');
  exact(CONFIG.authorization.phrase, REQUIRED_AUTHORIZATION_PHRASE, 'DOKE_COM_B04E_ATTEMPT_2_AUTHORIZATION_PHRASE_MISMATCH');
  exact(CONFIG.authorization.received, true, 'DOKE_COM_B04E_ATTEMPT_2_AUTHORIZATION_NOT_RECEIVED');
  exact(CONFIG.authorization.consumed, false, 'DOKE_COM_B04E_ATTEMPT_2_AUTHORIZATION_ALREADY_CONSUMED');
  exact(CONFIG.authorization.singleUse, true, 'DOKE_COM_B04E_ATTEMPT_2_AUTHORIZATION_NOT_SINGLE_USE');
  exact(CONFIG.authorization.reusableAfterFailure, false, 'DOKE_COM_B04E_ATTEMPT_2_AUTHORIZATION_REUSE_ENABLED');
  exact(CONFIG.execution.attempted, false, 'DOKE_COM_B04E_ATTEMPT_2_PRIOR_ATTEMPT_REQUIRES_NEW_AUTHORIZATION');
  exact(CONFIG.execution.workflowRerunAllowed, false, 'DOKE_COM_B04E_ATTEMPT_2_RERUN_MUST_REMAIN_BLOCKED');
  exact(CONFIG.previousAttempt.run, 31065331290, 'DOKE_COM_B04E_ATTEMPT_2_PREVIOUS_RUN_MISMATCH');
  exact(CONFIG.previousAttempt.authorizationConsumed, true, 'DOKE_COM_B04E_ATTEMPT_2_PREVIOUS_AUTHORIZATION_STATE_INVALID');
  exact(CONFIG.previousAttempt.executorStarted, false, 'DOKE_COM_B04E_ATTEMPT_2_PREVIOUS_EXECUTOR_STATE_INVALID');
  exact(CONFIG.target.environment, 'staging', 'DOKE_COM_B04E_ATTEMPT_2_TARGET_NOT_STAGING');
  exact(CONFIG.target.projectId, REQUIRED_PROJECT_ID, 'DOKE_COM_B04E_ATTEMPT_2_PROJECT_MISMATCH');
  exact(CONFIG.target.pullRequest, EXPECTED_PULL_REQUEST, 'DOKE_COM_B04E_ATTEMPT_2_PR_MISMATCH');
  exact(CONFIG.target.branch, EXPECTED_BRANCH, 'DOKE_COM_B04E_ATTEMPT_2_BRANCH_MISMATCH');
  exact(CONFIG.canary.syntheticOnly, true, 'DOKE_COM_B04E_ATTEMPT_2_SYNTHETIC_ONLY_REQUIRED');
  exact(CONFIG.canary.rollbackOnly, true, 'DOKE_COM_B04E_ATTEMPT_2_ROLLBACK_ONLY_REQUIRED');
  exact(CONFIG.canary.outerIsolation, 'serializable', 'DOKE_COM_B04E_ATTEMPT_2_SERIALIZABLE_REQUIRED');
  exact(CONFIG.canary.coreCompositionActivationMode, 'disabled', 'DOKE_COM_B04E_ATTEMPT_2_CORE_MUST_REMAIN_DISABLED');
  exact(CONFIG.effects.routeRegistrationAllowed, false, 'DOKE_COM_B04E_ATTEMPT_2_ROUTE_REGISTRATION_BLOCKED');
  exact(CONFIG.effects.runtimeDeploymentAllowed, false, 'DOKE_COM_B04E_ATTEMPT_2_RUNTIME_DEPLOYMENT_BLOCKED');
  exact(CONFIG.effects.productionAllowed, false, 'DOKE_COM_B04E_ATTEMPT_2_PRODUCTION_BLOCKED');
  exact(env.COM_B04E_ATTEMPT_2_AUTHORIZATION, REQUIRED_AUTHORIZATION_PHRASE, 'DOKE_COM_B04E_ATTEMPT_2_ENV_AUTHORIZATION_MISMATCH');
  exact(env.SUPABASE_PROJECT_REF, REQUIRED_PROJECT_ID, 'DOKE_COM_B04E_ATTEMPT_2_ENV_PROJECT_MISMATCH');
  exact(String(env.GITHUB_RUN_ATTEMPT || '1'), '1', 'DOKE_COM_B04E_ATTEMPT_2_WORKFLOW_RERUN_BLOCKED');
  if (!env.SUPABASE_ACCESS_TOKEN) fail('DOKE_COM_B04E_ATTEMPT_2_ACCESS_TOKEN_MISSING');
  if (!env.SUPABASE_DB_PASSWORD) fail('DOKE_COM_B04E_ATTEMPT_2_DB_PASSWORD_MISSING');
}
...
async function verifyPullRequest(env) {
  const pull = await fetchJson(
    ...
    { headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'doke-com-b04e-attempt-2-canary' } },
    'DOKE_COM_B04E_ATTEMPT_2_PR_PREFLIGHT_FAILED'
  );
  exact(pull.state, 'open', 'DOKE_COM_B04E_ATTEMPT_2_PR_NOT_OPEN');
  exact(pull.draft, true, 'DOKE_COM_B04E_ATTEMPT_2_PR_NOT_DRAFT');
  exact(pull.merged, false, 'DOKE_COM_B04E_ATTEMPT_2_PR_ALREADY_MERGED');
  exact(pull.auto_merge, null, 'DOKE_COM_B04E_ATTEMPT_2_AUTO_MERGE_ENABLED');
  exact(pull.head && pull.head.ref, EXPECTED_BRANCH, 'DOKE_COM_B04E_ATTEMPT_2_PR_BRANCH_MISMATCH');
  if (env.GITHUB_SHA) exact(pull.head && pull.head.sha, env.GITHUB_SHA, 'DOKE_COM_B04E_ATTEMPT_2_PR_SHA_MISMATCH');
  ...
  const project = await fetchJson(
    `https://api.supabase.com/v1/projects/${REQUIRED_PROJECT_ID}`,
    {
      headers: {
        ...
        'User-Agent': 'doke-com-b04e-attempt-2-canary'
      }
    },
    'DOKE_COM_B04E_ATTEMPT_2_PROJECT_PREFLIGHT_FAILED'
  );
  exact(project.id, REQUIRED_PROJECT_ID, 'DOKE_COM_B04E_ATTEMPT_2_PROJECT_ID_MISMATCH');
  exact(project.name, EXPECTED_PROJECT_NAME, 'DOKE_COM_B04E_ATTEMPT_2_PROJECT_NAME_MISMATCH');
  exact(project.status, 'ACTIVE_HEALTHY', 'DOKE_COM_B04E_ATTEMPT_2_PROJECT_NOT_HEALTHY');
  const region = String(project.region || '').trim().toLowerCase();
  if (!/^[a-z]{2}-[a-z]+-\d$/.test(region)) fail('DOKE_COM_B04E_ATTEMPT_2_PROJECT_REGION_INVALID');
  const directHost = project.database && project.database.host;
  if (directHost && directHost !== `db.${REQUIRED_PROJECT_ID}.supabase.co`) {
    fail('DOKE_COM_B04E_ATTEMPT_2_DATABASE_HOST_MISMATCH');
  }
...
async function connectStaging(project, password) {
  ...
  for (const host of [...new Set(candidates)]) {
    ...
    const pool = new Pool({
      ...
      application_name: 'doke-com-b04e-attempt-2-rollback-canary'
    });
    try {
  ...
  fail('DOKE_COM_B04E_ATTEMPT_2_DATABASE_CONNECTION_FAILED');
}
...
async function verifySchema(client) {
  const response = await client.query({
    name: 'com-b04e-attempt-2-schema-preflight',
    text: `select
      ...
      encode(digest('com-b04e-attempt-2-schema-preflight', 'sha256'), 'hex') ~ '^[a-f0-9]{64}$' as pgcrypto_digest_available`,
    values: [REQUIRED_MIGRATIONS]
  });
  const row = response.rows[0] || {};
  if (Object.values(row).some((value) => value !== true)) fail('DOKE_COM_B04E_ATTEMPT_2_SCHEMA_GATE_FAILED');
...
async function loadAuthenticatedActor(client) {
  ...
  limit 1`);
  const row = response.rows[0];
  if (!row) fail('DOKE_COM_B04E_ATTEMPT_2_VALID_AUTHENTICATED_SESSION_REQUIRED');
  if (!['aal1', 'aal2'].includes(row.aal)) fail('DOKE_COM_B04E_ATTEMPT_2_SUPPORTED_AAL_REQUIRED');
  return Object.freeze({
...
function createPgExecutor(client) {
  ...
  return Object.freeze({
    ...
    async rpc(name, args) {
      if (!allowed.has(name)) fail('DOKE_COM_B04E_ATTEMPT_2_RPC_NOT_ALLOWLISTED');
      try {
        if (name === 'com_moderation_load_case_v1') {
          const response = await client.query({
            name: 'com-b04e-attempt-2-load-canonical-case',
        ...
        const response = await client.query({
          name: 'com-b04e-attempt-2-commit-case-command',
...
function createTransactionGuard(client) {
  return Object.freeze({
    ...
    async assertActive() {
      ...
      const row = response.rows[0] || {};
      exact(row.current_user, 'service_role', 'DOKE_COM_B04E_ATTEMPT_2_SERVICE_ROLE_NOT_ACTIVE');
      exact(row.isolation, 'serializable', 'DOKE_COM_B04E_ATTEMPT_2_TRANSACTION_NOT_SERIALIZABLE');
      exact(row.read_only, 'off', 'DOKE_COM_B04E_ATTEMPT_2_TRANSACTION_MUST_ALLOW_ROLLBACK_SCOPED_WRITE');
      return true;
    }
  });
}
...
function createCanonicalContext(actorId) {
  return Object.freeze({
    ...
    policy: Object.freeze({
      status: 'approved',
      version: '2026.08.05-com-b04e-attempt-2-canary',
      fingerprint: moderation.sha256('com-b04e-attempt-2-approved-canary-policy'),
...
function assertCountDelta(before, inside) {
  ...
  for (const [key, delta] of Object.entries(expected)) {
    exact(inside[key] - before[key], delta, `DOKE_COM_B04E_ATTEMPT_2_COUNT_DELTA_${key.toUpperCase()}_INVALID`);
  }
  return expected;
}
...
async function executeCanary(env) {
  const report = {
    validationId: 'COM-B04E-ATTEMPT-2-AUTHENTICATED-ROLLBACK-ONLY-MODERATION-RUNTIME-CANARY',
    ...
    authorization: {
      ...
      receivedAt: CONFIG.authorization && CONFIG.authorization.receivedAt || null,
      singleUse: true,
      consumed: true,
  ...
  try {
    ...
    if (!serverNow || !Number.isFinite(Date.parse(serverNow))) fail('DOKE_COM_B04E_ATTEMPT_2_SERVER_CLOCK_REQUIRED');

    await client.query('SET LOCAL ROLE service_role');
    const executor = createPgExecutor(client);
    const canary = createModerationRollbackCanaryAttempt2({
    ...
    const request = {
      ...
      requestId: 'com-b04e-attempt-2-authenticated-rollback-only-canary',
    ...
    const rows = await client.query({
      name: 'com-b04e-attempt-2-inside-transaction-evidence',
      text: `select
        projection.revision,
    ...
    if (!row) fail('DOKE_COM_B04E_ATTEMPT_2_ROLLBACK_SCOPED_ROWS_MISSING');
    exact(Number(row.revision), 1, 'DOKE_COM_B04E_ATTEMPT_2_REVISION_INVALID');
    exact(row.ledger_head_hash, canaryResult.eventHash, 'DOKE_COM_B04E_ATTEMPT_2_LEDGER_HASH_INVALID');
    exact(row.evidence_kind, 'report_statement', 'DOKE_COM_B04E_ATTEMPT_2_EVIDENCE_KIND_INVALID');
    exact(row.opaque_reference, CONFIG.synthetic.initialEvidenceReference, 'DOKE_COM_B04E_ATTEMPT_2_EVIDENCE_REFERENCE_INVALID');
    exact(row.digest, moderation.sha256(CONFIG.synthetic.initialEvidenceDigestSeed), 'DOKE_COM_B04E_ATTEMPT_2_EVIDENCE_DIGEST_INVALID');

    await client.query('ROLLBACK');
    transactionOpen = false;

    const postflightCounts = await loadCounts(client);
    assertSameCounts(baselineCounts, postflightCounts, 'DOKE_COM_B04E_ATTEMPT_2_PERSISTENT_COUNTS_CHANGED');
    const residue = await client.query({
      name: 'com-b04e-attempt-2-postflight-residue',
      text: `select
    ...
    if (Object.values(residue.rows[0] || {}).some((value) => value !== true)) {
      fail('DOKE_COM_B04E_ATTEMPT_2_PERSISTENT_RESIDUE_DETECTED');
    }
    ...
    writeReport(report);
    console.log('COM-B04E attempt-2 authenticated rollback-only moderation runtime canary passed.');
