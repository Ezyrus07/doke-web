...
helper = """function canonicalizeProducerPayload(payload) {
    ...
    return Object.assign({}, payload, {
      ...
      sourceAuthority: shouldUseNotificationsApi() ? 'CANONICAL_REMOTE' : 'CANONICAL_LOCAL',
...
# Dynamic canonical semantics for status notifications while legacy type/category stay stable.
...
function assertEnvelope(payload, expected) {
  ...
  assert.equal(normalized.accepted, true, `${expected.eventType} must be accepted`);
...
(async () => {
  try {
    ...
    assert.equal(assertEnvelope(payload, { eventType: 'payment_confirmed', category: 'PAYMENTS' }).criticalOperational, true);
    ...
    assert.equal(assertEnvelope(payload, { eventType: 'payment_held', category: 'PAYMENTS' }).criticalOperational, true);

    const opened = await notifications.createDisputeOpened({ id: 'order-6', clientId: 'client-1', professionalId: 'pro-6' }, { id: 'pay-6', orderId: 'order-6' }, { id: 'dispute-6', orderId: 'order-6' }, {});
    assert.equal(opened.length, 2);
    assert.equal(opened[0].category, 'orders');
    assert.equal(assertEnvelope(opened[0], { eventType: 'dispute_opened', category: 'DISPUTES' }).criticalOperational, true);
    assertEnvelope(opened[1], { eventType: 'dispute_reported', category: 'DISPUTES' });

    const responded = await notifications.createDisputeResponded({ id: 'order-6', clientId: 'client-1', professionalId: 'pro-6' }, { orderId: 'order-6' }, { id: 'dispute-6', orderId: 'order-6' }, { actor: { id: 'pro-6', name: 'Profissional' } });
    assertEnvelope(responded[0], { eventType: 'dispute_responded', category: 'DISPUTES' });

    const resolved = await notifications.createDisputeResolved({ id: 'order-6', clientId: 'client-1', professionalId: 'pro-6' }, { orderId: 'order-6' }, { id: 'dispute-6', orderId: 'order-6', resolution: 'client' }, {});
    assert.equal(resolved.length, 2);
    resolved.forEach((item) => assertEnvelope(item, { eventType: 'dispute_resolved', category: 'DISPUTES' }));
    ...
    assert.ok(localPayloads.length >= 10);

    console.log('[ux-notif-002-producer-envelopes] ok');
    console.log('- native producer identity and semantic classification validated for messages, orders, proposals, payments and disputes');
  } finally {
    delete require.cache[require.resolve(eventPath)];
    delete require.cache[require.resolve(servicePath)];
