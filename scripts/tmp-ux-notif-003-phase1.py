...
INHERITED_IN_APP_TEST = ROOT / 'scripts/test-ux-notif-001-in-app-adapter.js'
...
text, escape_count = re.subn(r'^  const escapeHtml = .*\n', '', text, count=1, flags=re.MULTILINE)
if escape_count != 1:
...
if ensure_host_start < 0 or mark_read_start < 0:
    raise SystemExit('toast renderer authority anchors not found')
segment = text[ensure_host_start:mark_read_start]
icon_match = re.search(r"  const iconFor = \(payload\) => .*?;\n", segment)
if not icon_match:
    raise SystemExit('iconFor inside toast renderer block not found')
text = text[:ensure_host_start] + icon_match.group(0) + text[mark_read_start:]
...
script_pattern = re.compile(r'''(?P<indent>[ \t]*)<script\b[^>]*\bsrc=["']assets/js/features/in-app-notifications\.js(?:\?[^"']*)?["'][^>]*></script>''')
html_consumers = []
for html_path in sorted(ROOT.glob('*.html')):
    ...
    if 'assets/js/core/notification-toast.js' in source:
        raise SystemExit(f'{html_path}: notification-toast already loaded unexpectedly')
    ...
    if count != 1:
        raise SystemExit(f'{html_path}: expected one in-app script tag, found {count}')
    html_path.write_text(updated, encoding='utf-8')
    html_consumers.append(html_path.name)
if not html_consumers:
    raise SystemExit('no root HTML consumers found')

inherited = INHERITED_IN_APP_TEST.read_text(encoding='utf-8')
old_harness = """delete require.cache[require.resolve('../assets/js/core/notification-center.js')];
...
new_harness = """delete require.cache[require.resolve('../assets/js/core/notification-center.js')];
delete require.cache[require.resolve('../assets/js/core/notification-toast.js')];
delete require.cache[require.resolve('../assets/js/features/in-app-notifications.js')];
require('../assets/js/core/notification-center.js');
require('../assets/js/core/notification-toast.js');
require('../assets/js/features/in-app-notifications.js');"""
inherited = replace_once(inherited, old_harness, new_harness, 'migrate inherited in-app harness dependency')
INHERITED_IN_APP_TEST.write_text(inherited, encoding='utf-8')
