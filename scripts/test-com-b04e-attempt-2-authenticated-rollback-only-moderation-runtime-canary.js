...
const ids = Object.freeze({
  actor: '11111111-1111-4111-8112-111111111111',
...
function dependencies(overrides = {}) {
  ...
  const contextLoader = {
    ...
    async load({ actorId }) {
      return {
        ...
        policy: {
          status: 'approved',
          version: '2026.08.05-canary-attempt-2',
          fingerprint: moderation.sha256('com-b04e-attempt-2-policy'),
  ...
  const clock = {
    ...
    async now() { return '2026-08-06T02:39:00.000Z'; }
  };
  return {
...
function request() {
  return {
    ...
    requestId: 'com-b04e-attempt-2-local-conformance',
    envelope: {
      command: 'open_case',
      ...
      payload: {
        ...
        initialEvidenceRef: 'opaque:moderation:canary:b04e:attempt-2:report',
        initialEvidenceDigest: moderation.sha256('com-b04e-attempt-2-initial-evidence')
      }
    }
  };
}
...
(async () => {
  ...
  const canary = createModerationRollbackCanaryAttempt2(deps);
  ...
  equal(commit.p_evidence_record.reference, 'opaque:moderation:canary:b04e:attempt-2:report', 'opaque evidence reference');
  ...
  await assert.rejects(canary.execute(request()), /COM_B04E_ATTEMPT_2_AUTHORIZATION_ALREADY_CONSUMED/); checks += 1;
  assert.throws(() => createModerationRollbackCanaryAttempt2(dependencies({
    authorization: authorization({ phrase: 'wrong' })
  })), /COM_B04E_ATTEMPT_2_AUTHORIZATION_PHRASE_MISMATCH/); checks += 1;
  assert.throws(() => createModerationRollbackCanaryAttempt2(dependencies({
    authorization: authorization({ consumed: true })
  })), /COM_B04E_ATTEMPT_2_AUTHORIZATION_ALREADY_CONSUMED/); checks += 1;
  assert.throws(() => createModerationRollbackCanaryAttempt2(dependencies({
    transactionGuard: { ...dependencies().transactionGuard, environment: 'production' }
  })), /COM_B04E_ATTEMPT_2_STAGING_TRANSACTION_REQUIRED/); checks += 1;
  assert.throws(() => createModerationRollbackCanaryAttempt2(dependencies({
    executor: { ...dependencies().executor, environment: 'staging_live' }
  })), /COM_B04E_ATTEMPT_2_STAGING_CANARY_EXECUTOR_REQUIRED/); checks += 1;
  assert.throws(() => createModerationRollbackCanaryAttempt2(dependencies({
    transactionGuard: { ...dependencies().transactionGuard, rollbackOnly: false }
  })), /COM_B04E_ATTEMPT_2_ROLLBACK_ONLY_TRANSACTION_REQUIRED/); checks += 1;

  equal(checks, 35, 'deterministic check count');
  console.log(`COM-B04E attempt-2 conformance passed: ${checks}/${checks}`);
})().catch((error) => {
  console.error(error);
  process.exit(1);
});
