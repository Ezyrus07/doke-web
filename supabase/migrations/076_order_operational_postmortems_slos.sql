-- Doke: post-incident records and operational SLO projection for order operations.
-- Timeline metrics are derived from immutable incident timestamps; operators only add analysis.
...
create table if not exists private.order_operational_postmortems (
  ...
  root_cause_category text check (root_cause_category is null or root_cause_category in (
    'application','database','integration','infrastructure','configuration','capacity','process','unknown'
  )),
...
begin
  ...
  insert into private.order_operational_postmortems (
    ...
    new.escalation_count,
    new.alert_type,
    v_resolved,
    v_resolved
  )
  on conflict (alert_id, cycle_count) do update set
...
begin
  ...
  if p_postmortem_id is null then raise exception using errcode='22023', message='DOKE_ORDER_POSTMORTEM_ID_REQUIRED'; end if;
  ...
  select * into v_row from private.order_operational_postmortems where id = p_postmortem_id for update;
  if not found then raise exception using errcode='P0002', message='DOKE_ORDER_POSTMORTEM_NOT_FOUND'; end if;
...
revoke all on function public.mutate_order_operational_postmortem_internal(uuid,uuid,text,text,text,text,boolean)
  from public, anon, authenticated;
grant execute on function public.mutate_order_operational_postmortem_internal(uuid,uuid,text,text,text,text,boolean)
  to service_role;
...
revoke all on function public.get_order_operational_slos_internal(uuid,integer,integer)
  from public, anon, authenticated;
grant execute on function public.get_order_operational_slos_internal(uuid,integer,integer)
  to service_role;
