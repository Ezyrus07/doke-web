-- ANA-001 / ANA-A10 compatibility follow-up.
-- The base migration is immutable after staging application. PostgreSQL LEAST/GREATEST
-- are conditional expressions and cannot be schema-qualified.
  ...
  projection_summary as (
    select
      ...
      coalesce(pg_catalog.floor(pg_catalog.sum(
        extract(epoch from (
          least(i.active_until,p_window_end) -
          greatest(i.active_from,p_window_start)
        ))
...
comment on function public.compute_analytics_cat_liquidity_v1(timestamptz,timestamptz,text,text) is
  'ANA-A10 read-only CAT-A06 liquidity projection. Compatibility follow-up removes invalid pg_catalog qualification from LEAST/GREATEST without changing semantics.';
