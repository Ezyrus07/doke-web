-- Doke: harden cron health detection for transient startup states.
-- pg_cron may expose connecting/sending/running rows before a stable start_time is visible.
...
begin
  ...
  begin
    ...
    select d.status,
           case
             when lower(coalesce(d.status, '')) in ('connecting', 'sending', 'running') then coalesce(d.start_time, p_observed_at)
             else d.start_time
           end
...
begin
  ...
  if not v_cron_active
     ...
     or v_cron_last_status not in ('succeeded', 'success', 'running', 'sending', 'connecting') then
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
...
comment on function private.classify_order_operational_alerts(jsonb, timestamptz) is
  'Classifies five order-worker health signals; fresh connecting/sending/running cron invocations are healthy until the stale threshold expires.';
