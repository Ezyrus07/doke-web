-- CAT-001 / CAT-A07: forward-only supply coverage baseline.
-- Repository candidate only until separately authorized for staging.
-- Does not rewrite CAT-A06 activation evidence and does not infer historical visibility.

begin;

create table if not exists private.cat_listing_supply_coverage_epochs_v1 (
  baseline_run_id uuid primary key,
  contract_id text not null,
  coverage_complete_from timestamptz not null,
  source_service_count bigint not null,
  baseline_event_count bigint not null,
  source_fingerprint text not null,
  ledger_fingerprint text not null,
  certification_state text not null,
  created_at timestamptz not null default pg_catalog.now(),
  constraint cat_listing_supply_coverage_epochs_contract_check
    check (contract_id = 'cat-a07-supply-coverage-baseline-v1'),
  constraint cat_listing_supply_coverage_epochs_counts_check
    check (source_service_count >= 0 and baseline_event_count >= 0 and source_service_count = baseline_event_count),
  constraint cat_listing_supply_coverage_epochs_state_check
    check (certification_state = 'certified'),
  constraint cat_listing_supply_coverage_epochs_source_hash_check
    check (source_fingerprint ~ '^[0-9a-f]{64}$'),
  constraint cat_listing_supply_coverage_epochs_ledger_hash_check
    check (ledger_fingerprint ~ '^[0-9a-f]{64}$'),
  unique (coverage_complete_from)
);

create index if not exists idx_cat_listing_supply_coverage_epochs_time
  on private.cat_listing_supply_coverage_epochs_v1(coverage_complete_from desc);

alter table private.cat_listing_supply_coverage_epochs_v1 enable row level security;

revoke all on table private.cat_listing_supply_coverage_epochs_v1
  from public, anon, authenticated, service_role;
grant select on table private.cat_listing_supply_coverage_epochs_v1 to service_role;
...
comment on table private.cat_listing_supply_coverage_epochs_v1 is
  'CAT-A07 append-only certified forward coverage epochs. An epoch proves complete CAT supply state only from coverage_complete_from forward.';
comment on function public.run_cat_listing_supply_coverage_baseline_v1(uuid) is
  'CAT-A07 server-only one-shot forward baseline. Serializes CAT source/ledger, aborts on drift, appends one activation_baseline event per current service, and never rewrites CAT-A06 activation history.';
