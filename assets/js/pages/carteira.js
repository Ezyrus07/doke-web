(() => {
  const initWallet = () => {
    ...
    const historyFilters = Array.from(root.querySelectorAll('[data-wallet-history-filter]'));
    const historyItems = Array.from(root.querySelectorAll('[data-wallet-history-item]'));
    ...
    const applyHistoryFilter = (filterKey = 'all') => {
      historyFilters.forEach((button) => {
        button.classList.toggle('is-active', button.dataset.walletHistoryFilter === filterKey);
      });

      historyItems.forEach((item) => {
        const itemType = item.dataset.historyType || 'all';
        const shouldShow = filterKey === 'all' || itemType === filterKey;
    ...
    historyFilters.forEach((button) => {
      button.addEventListener('click', () => {
        applyHistoryFilter(button.dataset.walletHistoryFilter || 'all');
      });
    });
