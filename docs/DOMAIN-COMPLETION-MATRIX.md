...
- SCHED-B02B runs 30675212259 and 30675383979 stopped in read-only preflight because staging did not contain an active synthetic client matching the required identity projection; both mutation steps were skipped and staging mutations remained zero. Run 30675813762 passed preflight and transactional fixture provisioning, then failed closed while inserting the synthetic order fixture; ROLLBACK and all 11 residue counters plus authority-count equality were independently verified. Attempt 7 adds a read-only gate for the corrected CAT-B04 snapshot trigger and exposes only sanitized DOKE error codes.
...
**Evidência estática observada:** 2353 arquivos no escopo; 505 referências a localStorage; 151 a sessionStorage; 880 referências mock; 593 referências de rede/Supabase; 89 marcadores de implementação pendente.
