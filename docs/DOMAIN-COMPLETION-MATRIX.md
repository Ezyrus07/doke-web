...
- Obtain a fresh ANA-A07/A09 controlled staging canary authorization against the post-service-eligibility-remediation PR HEAD with identityAuthority=seed002-email-resolved, serviceEligibilityAuthority=approved-service-version and fixtureMode=synthetic-approved-version; then execute validation 046 plus the transient second-session late-fact protocol with mandatory cleanup and no maturity promotion.
