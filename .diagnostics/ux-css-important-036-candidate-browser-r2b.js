...
const RESULT = '.diagnostics/ux-css-important-036-candidate-browser-r2b.json';
...
const out = {
  boundary: 'UX-CSS-IMPORTANT-036', proof: 'candidate-browser-r2b', certified035Head: CERTIFIED_HEAD,
  ...
  readiness: 'navigation-commit + body-attached + deterministic-settle',
  historicalR2Preserved: true,
    ...
    for (const pageName of PAGES) {
      for (const width of WIDTHS) {
        ...
        await page.route('**/*', async route => {
          ...
          if (u.startsWith('http://127.0.0.1:4173/') || u.startsWith('data:') || u.startsWith('blob:')) await route.continue();
...
(async()=>{
  ...
  try {
    ...
    out.diffs = {
      ...
      targetLoad:diffs.filter(d=>d.type==='target-load').length, sample:diffs.slice(0,50)
    };
