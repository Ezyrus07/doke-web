...
Toda página ativa, endpoint, tabela e fluxo crítico aparece na matriz, com autoridade, estado atual, dependências e teste associado.
...
Esta é a próxima fase prática. O trabalho de error budget continua no plano, mas não deve vir antes do fechamento da superfície de segurança já identificada.
...
Já existem worker, alertas, escalonamento, runbooks, pós-incidente e SLOs de pedidos.
...
O próximo passo anteriormente sugerido era implementar error budgets e proteção de mudanças. Esse trabalho é correto, mas não é mais a prioridade imediata após a visão completa do projeto.

A nova sequência responsável é:

```text
1. Segurança e autoridade dos dados
2. Identidade real
3. Fechamento do fluxo transacional completo
4. Pagamentos e conciliação
5. Mensagens/notificações/comunidade remotas
6. Backoffice e qualidade
7. Error budgets e proteção de mudanças antes do beta
```

Os SLOs e a operação de pedidos já construídos serão preservados. Apenas evitaremos aprofundar um subsistema enquanto os fluxos centrais ainda não estão integralmente conectados.
