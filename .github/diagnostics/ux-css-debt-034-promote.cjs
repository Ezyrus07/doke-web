...
assert(Buffer.byteLength(before)===42219,`parent bytes drift ${Buffer.byteLength(before)}`);
assert(p.declarations.length===734,`decl drift ${p.declarations.length}`);
assert(p.dead.length===53,`dead drift ${p.dead.length}`);
assert(new Set(p.dead.map(d=>d.selector)).size===18,'selector drift');
assert(p.dead.filter(d=>d.value===d.winnerValue&&d.important===d.winnerImportant).length===11,'identical drift');
assert(p.dead.every(d=>!d.important&&!d.winnerImportant),'important drift');
const contexts=[...new Set(p.dead.map(d=>d.context))].sort();
const expected=['','@media (max-width: 760px)','@media (max-width: 1180px)'].sort();
assert(JSON.stringify(contexts)===JSON.stringify(expected),'context drift '+JSON.stringify(contexts));
...
assert(a.declarations.length===681&&a.dead.length===0,`after drift ${a.declarations.length}/${a.dead.length}`);
...
const n=sh(`git diff --numstat -- ${TARGET}`).split(/\s+/);assert(n[0]==='0'&&n[1]==='53'&&n[2]===TARGET,'numstat drift '+n.join('/'));
sh('git diff --check');
console.log(`PROMOTION CANDIDATE PASS|hash=${hash}|bytes=${BYTES}|files=1|additions=0|deletions=53|decl=734->681|dead=53->0`);
