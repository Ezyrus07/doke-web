...
assert(Buffer.byteLength(before)===35773,`parent bytes drift ${Buffer.byteLength(before)}`);
assert(p.declarations.length===627,`decl drift ${p.declarations.length}`);
assert(p.dead.length===73,`dead drift ${p.dead.length}`);
assert(new Set(p.dead.map(d=>d.selector)).size===11,'selector drift');
assert(p.dead.filter(d=>d.value===d.winnerValue&&d.important===d.winnerImportant).length===60,'identical drift');
...
assert(cc.size===4&&cc.get('')===11&&cc.get('@media (max-width: 760px)')===6&&cc.get('@media (min-width: 761px) and (max-width: 1180px)')===42&&cc.get('@media (min-width: 1181px)')===14,'context drift '+JSON.stringify([...cc]));
...
assert(a.declarations.length===554&&a.dead.length===0,`after drift ${a.declarations.length}/${a.dead.length}`);
...
sh('git diff --check');
console.log(`PROMOTION CANDIDATE PASS|hash=${hash}|bytes=${BYTES}|files=1|additions=0|deletions=73|decl=627->554|dead=73->0`);
