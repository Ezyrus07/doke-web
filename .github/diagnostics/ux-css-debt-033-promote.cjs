...
assert(p.declarations.length===763,`decl drift ${p.declarations.length}`);
assert(p.dead.length===45,`dead drift ${p.dead.length}`);
assert(new Set(p.dead.map(d=>d.selector)).size===14,'selector drift');
...
assert(a.declarations.length===718&&a.dead.length===0,`after drift ${a.declarations.length}/${a.dead.length}`);
const hash=crypto.createHash('sha256').update(after).digest('hex');assert(hash===HASH,`candidate hash drift ${hash}`);
assert(Buffer.byteLength(after)===43826,`candidate bytes drift ${Buffer.byteLength(after)}`);
assert(sh(`git diff --name-only -- ${TARGET}`)===TARGET,'scope drift');const n=sh(`git diff --numstat -- ${TARGET}`).split(/\s+/);assert(n[0]==='0'&&n[1]==='45'&&n[2]===TARGET,'numstat drift '+n.join('/'));sh('git diff --check');
console.log(`PROMOTION CANDIDATE PASS|hash=${hash}|bytes=43826|files=1|additions=0|deletions=45|decl=763->718|dead=45->0`);
