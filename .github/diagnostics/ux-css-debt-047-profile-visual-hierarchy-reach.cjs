...
const report=JSON.parse(fs.readFileSync('reports/generated/active-legacy-structures-report.json','utf8')),reached=[],rootMap=[];
for(const pa of report.pageAssets){const roots=(pa.css||[]).filter(f=>tracked.has(f)),seen=new Set(),stack=[...roots];let hit=false;while(stack.length){const f=stack.pop();if(seen.has(f)||!tracked.has(f))continue;seen.add(f);if(f===TARGET){hit=true;break;}for(const n of importsFor(f))stack.push(n);}if(hit){reached.push(pa.page);rootMap.push(`${pa.page}:${roots.join('+')}`);}}
reached.sort();assert(JSON.stringify(reached)===JSON.stringify(EXPECTED_PAGES),'page reach drift '+JSON.stringify(reached));
for(const page of EXPECTED_PAGES){const html=fs.readFileSync(page,'utf8');assert(/<body[^>]*class=["'][^"']*profile-page-shell/.test(html),'profile-page-shell missing '+page);assert(/data-profile-contract=["']clean-v1["']/.test(html),'clean-v1 contract missing '+page);}
...
console.log(`REACH AUTHORITY PASS|pages=${reached.join(',')}|directImporter=${EXPECTED_IMPORTER}|rootMap=${rootMap.sort().join(';')}|target=${TARGET}|affectedFamilies=1`);
