...
const TARGET = 'assets/css/pages/pedidos/mobile-layout-contract.css';
const FOUNDATION = 'assets/css/pages/pedidos-foundation.css';
const OPERATIONS = 'assets/css/pages/pedidos-runtime-operations.css';
...
function verifyReach() {
  ...
  assert(JSON.stringify(directImporters)===JSON.stringify([OPERATIONS]), `target importer drift ${JSON.stringify(directImporters)}`);
  assert(importsFor(FOUNDATION,tracked).includes(OPERATIONS), 'foundation operations import drift');
  const html=fs.readFileSync(PAGE,'utf8');
  assert((html.match(/assets\/css\/pages\/pedidos-foundation\.css\?v=/g)||[]).length===1, 'pedidos foundation link drift');
  const pages=[];
  ...
  console.log(`STATIC REACH PASS|chain=${PAGE}->${FOUNDATION}->${OPERATIONS}->${TARGET}|pages=1`);
}

function verifyDead(parsed) {
  assert(parsed.declarations.length===273, `declarations drift ${parsed.declarations.length}`);
  ...
  assert(identical===30 && changed===43, `identity drift ${identical}/${changed}`);
  assert(parsed.dead.every(d=>!d.important&&!d.winnerImportant), 'important drift in dead rows');
  assert(parsed.dead.every(d=>d.context==='@media (max-width: 760px)'), `context drift ${JSON.stringify([...new Set(parsed.dead.map(d=>d.context))])}`);
  for(const d of parsed.dead) assert(Number.isInteger(d.winnerLine) && d.winnerLine>d.line, `static winner drift line=${d.line}`);
  console.log('DEAD ROWS PASS|decl=273|dead=73|selectors=17|identical=30|changed=43|contexts=1|max760=73|staticCascadeRows=73');
}
...
function removeDeadRows(parsed) {
  ...
  assert(after.declarations.length===200, `after declarations ${after.declarations.length}`);
  assert(after.dead.length===0, `after dead ${after.dead.length}`);
  console.log('EPHEMERAL DELTA PASS|file=mobile-layout-contract.css|additions=0|deletions=73|decl=273->200|dead=73->0');
}
