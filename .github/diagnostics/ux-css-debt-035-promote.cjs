...
assert(Buffer.byteLength(before)===29368,`parent bytes drift ${Buffer.byteLength(before)}`);
assert(p.declarations.length===545,`decl drift ${p.declarations.length}`);
assert(p.dead.length===54,`dead drift ${p.dead.length}`);
assert(new Set(p.dead.map(d=>d.selector)).size===13,'selector drift');
assert(p.dead.filter(d=>d.value===d.winnerValue&&d.important===d.winnerImportant).length===20,'identical drift');
...
assert(cc.size===2&&cc.get('')===52&&cc.get('@media (max-width: 640px)')===2,'context drift '+JSON.stringify([...cc]));
...
assert(a.declarations.length===491&&a.dead.length===0,`after drift ${a.declarations.length}/${a.dead.length}`);
...
const n=sh(`git diff --numstat -- ${TARGET}`).split(/\s+/);assert(n[0]==='0'&&n[1]==='54'&&n[2]===TARGET,'numstat drift '+n.join('/'));
sh('git diff --check');
console.log(`PROMOTION CANDIDATE PASS|hash=${hash}|bytes=${BYTES}|files=1|additions=0|deletions=54|decl=545->491|dead=54->0`);
