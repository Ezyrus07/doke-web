...
const TARGET = 'assets/css/pages/carteira/responsive-contract.css';
const IMPORTER = 'assets/css/pages/carteira-runtime-page.css';
const FOUNDATION = 'assets/css/pages/carteira-foundation.css';
const PAGE = 'carteira.html';
const TARGET_SHA = process.env.TARGET_SHA;
const TARGET_BLOB = process.env.TARGET_BLOB;
const VIEWPORTS = [360, 379, 380, 381, 759, 760, 761, 762, 899, 900, 901, 1024, 1280, 1440];
...
function verifyReach() {
  ...
  const directImporters=sh("git grep -l -F 'carteira/responsive-contract.css?v=' -- assets/css || true").split(/\r?\n/).filter(Boolean).sort();
  assert(JSON.stringify(directImporters)===JSON.stringify([IMPORTER]), `target importer drift ${JSON.stringify(directImporters)}`);
  assert(importsFor(IMPORTER,tracked).includes(TARGET), 'carteira-runtime-page missing target import');
  assert(importsFor(FOUNDATION,tracked).includes(IMPORTER), 'carteira-foundation missing runtime-page import');
  const html=fs.readFileSync(PAGE,'utf8');
  assert((html.match(/assets\/css\/pages\/carteira-foundation\.css\?v=/g)||[]).length===1, 'carteira foundation link drift');
  const pages=[];
  ...
  console.log(`STATIC REACH PASS|chain=${PAGE}->${FOUNDATION}->${IMPORTER}->${TARGET}|pages=1`);
}

function verifyDead(parsed) {
  assert(parsed.declarations.length===763, `declarations drift ${parsed.declarations.length}`);
  assert(parsed.dead.length===45, `dead drift ${parsed.dead.length}`);
  ...
  assert(selectors.length===14, `selector drift ${selectors.length}`);
  assert(identical===21 && changed===24, `identity drift ${identical}/${changed}`);
  ...
  console.log('DEAD ROWS PASS|decl=763|dead=45|selectors=14|identical=21|changed=24|contexts=4');
}
...
function removeDeadRows(parsed) {
  ...
  assert(stat[0]==='0' && stat[1]==='45' && stat[2]===TARGET, `unexpected diff ${stat.join(' ')}`);
  const after=parse(fs.readFileSync(TARGET,'utf8'));
  assert(after.declarations.length===718, `after declarations ${after.declarations.length}`);
  assert(after.dead.length===0, `after dead ${after.dead.length}`);
  console.log('EPHEMERAL DELTA PASS|file=carteira/responsive-contract.css|additions=0|deletions=45|decl=763->718|dead=45->0');
}
...
async function capture(browser, tag, specs) {
  ...
  for(const width of VIEWPORTS){
    ...
    await page.evaluate(()=>{
      ...
      if(pageRoot){pageRoot.setAttribute('data-view-state','ready');pageRoot.setAttribute('aria-busy','false');}
      const sk=document.querySelector('[data-wallet-hydration-skeleton]'); if(sk) sk.hidden=true;
      const ready=document.querySelector('[data-wallet-hydration-ready]'); if(ready){ready.hidden=false;ready.removeAttribute('aria-hidden');}
    });
    ...
    const snap=await page.evaluate((specs)=>{
      ...
      return {
        rows,
        media:{max380:matchMedia('(max-width: 380px)').matches,max760:matchMedia('(max-width: 760px)').matches,tablet:matchMedia('(min-width: 761px) and (max-width: 900px)').matches,max900:matchMedia('(max-width: 900px)').matches},
  ...
  for(const [sel,count] of seenCounts) assert(count>0,`real DOM selector absent across viewports: ${sel}`);
  return out;
}
...
(async()=>{
  ...
  try{
    ...
    console.log(`CASCADE RUNTIME PARITY PASS|viewports=${VIEWPORTS.join(',')}|runtimeDiffs=0|overflow=0|boundaries=380/760/761/900|selectors=${specs.length}`);
  } finally {
    server.kill('SIGTERM');
  }
