...
function verifyDead(parsed) {
  assert(parsed.declarations.length===627, `declarations drift ${parsed.declarations.length}`);
  assert(parsed.dead.length===73, `dead drift ${parsed.dead.length}`);
  ...
  assert(selectors.length===11, `selector drift ${selectors.length}`);
  assert(identical===60 && changed===13, `identity drift ${identical}/${changed}`);
  assert(parsed.dead.every(d=>!d.important&&!d.winnerImportant), 'important drift in dead rows');
  const counts=new Map();
  for(const d of parsed.dead) counts.set(d.context,(counts.get(d.context)||0)+1);
  const expected=new Map([
    ['',11],
    ['@media (max-width: 760px)',6],
    ['@media (min-width: 761px) and (max-width: 1180px)',42],
    ['@media (min-width: 1181px)',14]
  ]);
  assert(counts.size===expected.size, `context size drift ${JSON.stringify([...counts])}`);
  for(const [ctx,n] of expected) assert(counts.get(ctx)===n, `context drift ${ctx||'<global>'}=${counts.get(ctx)} expected=${n}`);
  for(const d of parsed.dead) assert(Number.isInteger(d.winnerLine) && d.winnerLine>d.line, `static winner drift line=${d.line}`);
  console.log('DEAD ROWS PASS|decl=627|dead=73|selectors=11|identical=60|changed=13|contexts=4|global=11|max760=6|tablet761to1180=42|min1181=14|staticCascadeRows=73');
}
...
function removeDeadRows(parsed) {
  ...
  assert(stat[0]==='0' && stat[1]==='73' && stat[2]===TARGET, `unexpected diff ${stat.join(' ')}`);
  const after=parse(fs.readFileSync(TARGET,'utf8'));
  assert(after.declarations.length===554, `after declarations ${after.declarations.length}`);
  assert(after.dead.length===0, `after dead ${after.dead.length}`);
  console.log('EPHEMERAL DELTA PASS|file=settings-workspace-contract.css|additions=0|deletions=73|decl=627->554|dead=73->0');
}
    ...
    const snap=await page.evaluate((specs)=>{
      ...
      return {
        ...
        media:{
          ...
          tablet:matchMedia('(min-width: 761px) and (max-width: 1180px)').matches,
          min1181:matchMedia('(min-width: 1181px)').matches
        },
...
(async()=>{
  ...
  try{
    ...
    console.log(`CASCADE RUNTIME PARITY PASS|viewports=${VIEWPORTS.join(',')}|runtimeDiffs=0|overflow=0|boundaries=760/761/1180/1181|selectors=${specs.length}|domPresent=${before.coverage.present}|domAbsent=${before.coverage.absent.length}|staticCascadeRows=73`);
  } finally {
    server.kill('SIGTERM');
  }
