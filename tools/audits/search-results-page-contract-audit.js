#!/usr/bin/env node
/**
 * Doke Search/Resultados Page Contract Audit
 * Read-only check: flags shared component anatomy inside resultados/search-results page CSS.
 */
...
const scopeContains = [
  ...
  "assets/css/pages/resultados/",
  "assets/css/pages/results/"
];
...
const output = path.join(reportsDir, "search-results-page-contract-audit.json");
fs.writeFileSync(output, JSON.stringify({ generatedAt: new Date().toISOString(), violations }, null, 2));

console.log(`Search/results page contract audit: ${violations.length} violations found.`);
